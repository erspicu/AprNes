...
**生活比喻**：把 APU 想成一支 5 人樂團，每個人手上有不同樂器：

- **Pulse 1 / Pulse 2**：兩支方波吹奏者，能調整音高跟「音色形狀」(duty cycle 可選 12.5% / 25% / 50% / 75% 四種)，主要負責**主旋律跟配音**。
- **Triangle**：三角波貝斯手，音色比方波柔和，沒有音量調整（要嘛全開要嘛靜音），主要負責**bass line**。
- **Noise**：白噪音打擊樂手，模擬鼓聲、爆炸、雜訊。
- **DMC** (Delta Modulation Channel)：取樣播放器，可以播放預錄的 PCM 取樣（極粗糙、5KHz 上下），常用來播語音 (《Mike Tyson's Punch-Out!!》的「叮咚」鈴聲) 或低音鼓。

每個聲道**獨立執行**，由一個 4 拍指揮 (frame counter) 在固定週期通知大家「該換段了」。

```
APU                                          
├── Pulse 1   (方波 A)  ─→  ┐
├── Pulse 2   (方波 B)  ─→  │
├── Triangle  (三角波)  ─→  ├─→ 混音器 (非線性 lookup) ─→ DAC ─→ 喇叭
├── Noise     (雜訊)    ─→  │
└── DMC       (取樣)    ─→  ┘
                            ▲
                     混音公式不是線性相加！
