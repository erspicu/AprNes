...
> **最後修訂**: 2026-03-26
>
> **修正說明** (2026-03-26)：新增 Symmetric I/Q 解調模式切換功能——預設使用 1960 年代消費級電視的對稱正交解調（I/Q 均為 18-tap, ~0.5 MHz），可在類比設定介面切換回 1953 年原始 NTSC 標準的非對稱 I/Q（I=18-tap, Q=54-tap）。
>
> **修正說明** (2026-03-23)：根據 NESdev 論壇回饋，修正以下 5 項文件描述錯誤：
> 1. **BrightnessBoost**：移除「CRT 電路透過增加驅動電壓來補償」的錯誤說法，改為高斯光束重疊自然填充間隙、boost 補償離散模擬的亮度損失
> 2. **I/Q 頻寬不對稱**：~~補充說明目前實作符合 1953 年 NTSC 原始規範~~（已實作切換功能，預設為 1960 年代對稱模式）
> 3. **隔行場抖動 (Interlace Jitter)**：標註為美學效果——NES 輸出 240p 逐行訊號，不存在場間抖動的物理機制
> 4. **磷光體餘輝 (Phosphor Persistence)**：標註為藝術增強效果，補充真實 P22 磷光體衰減時間數據（遠快於模擬值）
> 5. **電子槍匯聚偏差 (Beam Convergence)**：補充說明因電視品質與老化程度而異，引用 SMPTE RP 167 可接受公差
>
> 以上為文件描述的修正。有錯誤的地方，之後會視情況修正程式實作。
...
- **Hann 窗 FIR 相干解調**：亮度（Y）用 6-tap 窗，色差 I 用 18-tap，Q 預設也用 18-tap（1960 年代對稱模式），可切換為 54-tap（1953 年非對稱模式）——這和真實電視晶片裡的帶通濾波器做的是同一件事
...
Level 3:  NES 調色盤 ──→ 21MHz 波形 ──→ FIR 解調 ──→ 線性 RGB ──┬→ CRT 顯示層 ──→ 螢幕 (CRT ON)
                         Stage 1                                  └→ 直接輸出 ──→ 螢幕 (CRT OFF)
                                                                      Stage 2
```
...
Level 3 的管線由兩個**完全獨立**的階段組成：

```
Stage 1 (Ntsc.cs)                      Stage 2 (CrtScreen.cs)
訊號生成 → 解調 → 線性 RGB     ──→     掃描線 → Bloom → 蔭罩 → 餘輝 → 曲率 → 輸出
              ↓ (CRT OFF)
         直接輸出至螢幕
```

Stage 1 負責「NES 到電視訊號線」的物理過程——波形生成、相干解調、IIR 濾波、振鈴、RF 干擾等。Stage 2 負責「電視訊號線到螢幕上的光」的物理過程——掃描線光斑、Bloom、蔭罩、磷光餘輝、曲率等。

AprNes 提供一個 **CRT 顯示開關**（`CrtEnabled`），讓使用者可以在 Level 3 下獨立關閉 Stage 2。這是一般模擬器不存在的選項——因為它們通常把「NTSC 濾鏡」和「CRT 效果」視為一個整體，要開一起開、要關一起關。

**Level 3 + CRT ON**：完整的兩階段物理模擬，從訊號到螢幕的全鏈路，是最接近真實 CRT 體驗的模式。

**Level 3 + CRT OFF**：只執行 Stage 1 的高精度 NTSC 訊號解調，跳過所有 CRT 電視端效果。畫面呈現的是「如果用最完美的監視器直接觀看 NTSC 訊號」的結果——有完整的 dot crawl、色彩模糊、振鈴、RF 干擾，但沒有掃描線、Bloom、蔭罩、螢幕曲率。

這個分離設計帶來幾個獨特的優勢：

- **精確的效果控制**：使用者可以享受高精度的 NTSC 色彩而不被掃描線干擾視線，或者反過來，搭配不同端子觀察 CRT 效果的差異
- **效能彈性**：Stage 2 包含多項像素級後處理（蔭罩、曲率重映射、磷光衰減），關閉它可以顯著降低運算量，同時保留 Stage 1 的全部訊號層物理效果
- **組合自由度**：Level 2（快速 IIR）、Level 3 + CRT OFF（精確解調無 CRT）、Level 3 + CRT ON（完整物理）三種模式覆蓋了從效能優先到擬真優先的完整光譜，而且每一種都是物理一致的——不是拼湊的濾鏡組合
...
| 訊號/CRT 獨立開關 | 無 | 無 | 手動拆裝 | — | **Stage 2 可獨立開關** |
...
1953 年原始 NTSC 標準（FCC Title 47 §73.682）規定 I 和 Q 的頻寬不同——I：0–1.3 MHz，Q：0–0.5 MHz。然而，如 Charles Poynton 在《Digital Video and HD》（第二版，§22.5）中所記載，到了 1960 年代初期，幾乎所有消費級電視都改用對稱正交解調，兩個色度軸均使用約 0.5 MHz 頻寬（B-Y/R-Y 而非 I/Q），因為非對稱解調成本更高且感知效益微乎其微。

模擬器提供 **Symmetric I/Q** 切換選項（位於類比設定介面的 NTSC 區域）：
- **預設開啟（1960 年代對稱模式）**：I 和 Q 均使用 18-tap FIR（~0.5 MHz），符合 NES 時代（1985–1995 年）消費級電視的實際做法，也與 Blargg 廣泛使用的 NES NTSC 濾波器一致
- **關閉（1953 年非對稱模式）**：I 使用 18-tap，Q 使用 54-tap（~0.5 MHz vs ~0.19 MHz），忠實重現原始 NTSC 標準的不對稱性

預設選擇 1960 年代模式的理由：NES 玩家使用的電視幾乎都是 1960 年代以後的消費機型，對稱解調更貼近真實遊玩體驗。對稱模式的 Q 濾波器較短（18-tap vs 54-tap），在高解析度下也有約 2-3% 的效能優勢。
...
掃描線間的黑溝會讓畫面整體看起來比原始訊號暗。在真實 CRT 上，電子束的高斯光束分布尾部會擴展到預定掃描線之外，相鄰掃描線在物理上產生重疊，因此黑溝不像離散數位模擬中那麼明顯。模擬器用 BrightnessBoost 係數來補償離散模擬固有的亮度損失，這是 CRT shader 的標準做法（CRT-Royale、CRT-Geom 等都使用類似的亮度補償係數），確保類比模式的整體亮度和非類比模式一致。
...
#### 磷光體餘輝 (Phosphor Persistence)【藝術增強效果】
CRT 螢幕上的 P22 磷光體（彩色 CRT 電視的標準）衰減到 10% 亮度的大約時間為：紅色 (Y₂O₂S:Eu) 約 1–3 ms、綠色 (ZnS:Cu,Al) 約 1 ms、藍色 (ZnS:Ag) 約 20–60 μs。在 60 Hz NTSC 下，一幀的時間是 16.7 ms，經過一整幀後即使是最慢的成分也已衰減至遠低於 1%——基本上不可見。模擬器實作的 PhosphorDecay = 0.6（每幀 60% 留存）相比真實 P22 物理特性是大幅誇大的。本功能為藝術增強效果，類似許多 CRT shader 中的「殘影」選項，提供某些使用者喜愛的視覺風格，但並非對真實 P22 磷光體行為的忠實重現。

#### 隔行場抖動 (Interlace Jitter)【美學效果】
NES 輸出 240p 逐行訊號（每幀 262 行，非隔行），每一幀都從相同的垂直位置開始——沒有半掃描線偏移的交替場。CRT 垂直偏轉電路只是跟隨同步脈衝，而 240p 的同步脈衝每幀都相同，因此在逐行訊號上不存在「場間抖動」的物理機制。本功能為純粹的美學／藝術效果，而非硬體行為的忠實模擬。若追求真實的緩慢視覺振盪效果，應模擬市電頻率（60 Hz）與 NES 垂直同步頻率（60.0988 Hz）之間的差頻所造成的約 17 秒差拍週期（即「電源紋波條紋 hum bar」效果）。
...
CRT 電視有三支電子槍分別負責紅、綠、藍。理想情況下，三束電子應該精確地打在同一個位置。高品質 CRT 電視配備靜態和動態會聚校正，校準良好的電視會顯示最小的會聚誤差。然而，許多便宜的消費級電視（大多數 NES 玩家使用的那種）會聚效果不佳，特別是在邊緣和角落；會聚性能也會隨著元件老化而退化。即使校準良好的電視在螢幕邊緣通常仍有 0.5–2mm 的會聚偏差（根據 SMPTE RP 167 的可接受公差）。預設的 ConvergenceStrength = 2.0 像素對於保養良好的電視來說偏高，但對於老化或便宜的電視來說是合理的。螢幕中央的匯聚最好，越靠近邊緣偏差越大，此參數可調整（包括 0 = 關閉）。
...
所有效果均可透過參數調整強度或完全關閉。搭配三層級渲染和 CRT 獨立開關，使用者可以在「原汁原味的復古感」和「乾淨的現代畫面」之間，以及「只要 NTSC 色彩」和「完整 CRT 體驗」之間，自由選擇最適合自己的組合。
