...
> 4. **磷光體餘輝 (Phosphor Persistence)**：預設值從 0.6 降至 0.15，接近真實 P22 磷光體特性（微弱餘輝），補充衰減時間數據
...
CRT 螢幕上的 P22 磷光體（彩色 CRT 電視的標準）衰減到 10% 亮度的大約時間為：紅色 (Y₂O₂S:Eu) 約 1–3 ms、綠色 (ZnS:Cu,Al) 約 1 ms、藍色 (ZnS:Ag) 約 20–60 μs。在 60 Hz NTSC 下，一幀的時間是 16.7 ms，經過一整幀後即使是最慢的成分也已衰減至遠低於 1%——基本上不可見。模擬器預設 PhosphorDecay = 0.15（每幀 15% 留存），僅在高速移動物體上產生極微弱的餘輝感——接近真實磷光體「肉眼幾乎不可察覺但物理上存在」的特性。使用者可自行調高此值以獲得更明顯的拖尾效果（如 0.4–0.6），但需注意高值屬於藝術誇大而非物理忠實。
