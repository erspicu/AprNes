...
AprNes 提供兩種類比渲染精度，使用者可以依照硬體效能自由切換：

- **Level 2（快速路徑）**：直接從 NES 調色盤查表取得 YIQ 值，搭配 6 點 dot crawl 相位 LUT，再透過 IIR 濾波輸出。速度是 Level 3 的數倍，仍然具備 dot crawl、色彩模糊、亮度響應延遲、振鈴等所有訊號特性
- **Level 3（UltraAnalog 物理路徑）**：完整生成 21.477 MHz 複合波形（每個像素 4 個取樣點，每條掃描線 1024 個浮點數），再用 Hann 窗 FIR 相干解調。頻域精度最高，色彩過渡最真實

兩個級別共用同一套 CRT 顯示層（Stage 2），所有掃描線、Bloom、蔭罩、曲率等效果完全一致。
...
| 特性 | Blargg LUT | CRT Shader | Blargg + Shader | **AprNes** |
|------|:----------:|:----------:|:---------------:|:----------:|
| NTSC 訊號域模擬 | 查表近似 | 無 | 查表近似 | **時域物理模擬** |
| IIR 狀態延續 | 無 | 無 | 無 | **全掃描線延續** |
| Dot crawl | 有 | 無 | 有 | **自然湧現** |
| 振鈴 / Gibbs | 無 | 無 | 無 | **二階動力學** |
| 掃描線 | 無 | 正弦疊加 | 正弦疊加 | **高斯光斑** |
| Bloom（亮度相依） | 無 | 部分 | 部分 | **物理連動** |
| 磷光體餘輝 | 無 | 少數支援 | 少數支援 | **逐通道衰減** |
| RF 干擾 | 無 | 無 | 無 | **4.5MHz 振盪器** |
| 端子切換 | 固定 | 無 | 手動切預設 | **同一管線切參數** |
| 訊號↔顯示連貫性 | — | — | 分離 | **兩階段物理一致** |

AprNes 的核心差異不在於「效果更多」，而在於**所有效果都是同一套物理方程式的不同面向**。調一個參數，所有相關現象自然跟著變——因為真實世界就是這樣運作的。
...
- **Stage 1**：模擬 NES 如何把畫面編碼成電視訊號，以及線材傳輸中的種種影響
- **Stage 2**：模擬 CRT 電視如何把訊號「畫」到螢幕上

使用者可以選擇三種連接方式，畫質由低到高：
...
在後續 14 項物理效果之前，AprNes 已經具備一套完整的 NTSC → CRT 類比模擬引擎。這些是整個系統的地基。
