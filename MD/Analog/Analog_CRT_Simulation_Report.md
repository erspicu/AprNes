...
> 2. **I/Q 頻寬不對稱**：補充說明目前實作符合 1953 年 NTSC 原始規範，但 1960 年代後消費級電視已改用對稱正交解調（引用 Poynton 文獻）
...
I 和 Q 的頻寬不同（I 較寬、Q 較窄），這是 1953 年原始 NTSC 標準（FCC Title 47 §73.682）的設計——I：0–1.3 MHz，Q：0–0.5 MHz。然而，如 Charles Poynton 在《Digital Video and HD》（第二版，§22.5）中所記載，到了 1960 年代初期，幾乎所有消費級電視都改用對稱正交解調，兩個色度軸均使用約 0.5 MHz 頻寬（B-Y/R-Y 而非 I/Q），因為非對稱解調成本更高且感知效益微乎其微。模擬器目前忠實重現了 1953 年標準的不對稱性（kWinI=18, kWinQ=54），但這不符合 NES 時代（1985–1995 年）消費級電視的實際做法。Blargg 廣泛使用的 NES NTSC 濾波器也採用對稱頻寬。
