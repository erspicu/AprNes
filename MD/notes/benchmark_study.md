...
```
.NET Framework 4.8.1 JIT  ████████████████████  422 FPS  (基準 100%)
...
| 執行環境 | FPS | 適用場景 | 不適場景 |
|----------|-----|---------|---------|
| .NET Framework 4.8.1 JIT | 422 | 現有 Windows 專案維護 | 高效能需求、跨平台 |
...
4. **.NET Framework 4.8.1 JIT 仍堪用**：422 FPS 對 NES 60 FPS 目標有 7× headroom，對使用者感受上沒有差異，但效能天花板比現代 .NET 低很多。
...
.NET Framework 4.8.1 保持原 scalar 路徑（無 `System.Runtime.Intrinsics`）。
...
#### 1. 執行環境選擇
- **.NET 10 RyuJIT** 是目前吞吐量最強的選擇（764 FPS），比 .NET Framework 4.8.1 快 **+81%**，比 .NET 8 快 +9%
- **Native AOT** 的定位不是「更快的 JIT」，而是「啟動速度 + 部署便利性」；在 CPU-bound 長時間運算上，AOT（550）輸給 JIT（764）約 28%
- **.NET Framework 4.8.1** 對 NES 60 FPS 目標有 7× headroom，使用者感知無差異，但效能天花板已固定
...
| 專案 | Runtime | 主要用途 |
|------|---------|---------|
| `AprNes` (.NET Fx 4.8.1) | JIT | 原始開發版本，最大相容性 |
