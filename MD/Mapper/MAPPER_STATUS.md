...
| 結果 | 數量 | Mapper 列表 |
|:----:|:----:|------------|
| ✅ 正常 | 65 | 000, 001, 002, 003, 004, 005, 007, 009, 010, 011, 013, 016, 018, 019, 020, 021, 022, 023, 024, 025, 026, 029, 032, 033, 034, 064, 065, 066, 067, 068, 069, 070, 071, 072, 075, 076, 077, 078, 079, 080, 082, 085, 087, 088, 089, 090, 093, 095, 097, **112**, 118, 119, 140, 152, 154, 159, **164**, **177**, 180, 184, 185, 206, 228, 232, **241** |
| ⚠️ 部分問題 | 0 | — |
| ❓ 待確認 | 4 | 153, 209, 210, 211 |
| **合計已實作** | **69** | |

### 2026-04-17 session 新增（4 個）
112 (Asder), 164 (Waixing 164), 177 (Henggedianzi), 241 (BxROM / Subor)

### 2026-04-17 session 嘗試失敗（暫緩）
| Mapper | 失敗原因 |
|:------:|--------|
| 012 / 074 / 191 / 192 / 194 | MMC3 + CHR-RAM/擴展 variant；共享 helper class 綠屏，需重構 base MMC3 (Mapper004) 為 virtual 可繼承 |
...
| 狀態 | 數量 | Mapper | 備註 |
|:----:|:----:|--------|------|
| ✅ 已實作 | 4 | 112, 164, 177, 241 | 2026-04-17 session 完成並驗證 |
| ⚠️ 嘗試過、待深度 debug | 6 | 012, 074, 096, 163, 176, 191 | 需要架構層改動或 3 次內修不好 |
| 🟡 ROM 不足/未啟動 | 3 | 126, 192, 194 | 未嘗試 |
| 🔴 無 ROM 或外設需求 | 2 | 153, 157 | 153 已做（❓），157 需條碼機外設 |

### 建議優先順序（剩餘項）

| 優先級 | Mapper | 理由 |
|:------:|:------:|------|
| **P1** | 163, 176 | 極高價值：南晶寶可夢、外星 RPG。需架構改動 |
| **P2** | 074, 191, 012 | MMC3 variant family，重構 Mapper004 可批次解決 |
| **P3** | 096 | 需新增 IMapper.NotifyVramAddressChange hook |
| **P5** | 126 | 低價值合卡 |
| **暫緩** | 157, 192, 194 | 無 ROM 或特殊外設 |

### ⚠️ 本 session 嘗試過但暫緩

| Mapper | 廠商 / 類別 | 嘗試結果 | 阻礙 |
|:------:|------------|---------|------|
| **074** | 漢化版 MMC3 | 綠屏 | MMC3 + CHR-RAM variant，共享 helper 失敗。需把 Mapper004 改為可繼承 |
| **191** | 漢化 MMC3（字庫） | 綠屏 | 同 074 |
| **192** | 漢化 MMC3 | 未測試 | 同 074 |
| **194** | 漢化 MMC3 | 未測試 | 同 074 |
| **012** | DBDROM (MMC3 + CHR ext) | 未嘗試 | 同 074 的變體 |
...
|:------:|------------|--------|---------|:----:|:----------:|
| **126** | Power Joy (台灣) | 合卡 | 多層選單 + 4MB+ banking | 低 | 1 |
...
| **192** | 漢化 MMC3 變體 | CHR-ROM/RAM 混合（類似 074） | 無 ROM 且 074 尚未解決 |
| **194** | 漢化 MMC3 變體 | 特殊鏡像控制 | 無 ROM 且 074 尚未解決 |

### 下一次可做方向

1. **Mapper004 refactor**：把 PRG/CHR banking + IRQ 方法標 `virtual`，開放 subclass override。一次解鎖 012/074/191/192/194。
