...
#
# Usage:
#   bash run_tests_AccuracyCoin_avalonia.sh                    # Full run (build + test + summary + report)
#   bash run_tests_AccuracyCoin_avalonia.sh --no-build         # Skip build
#   bash run_tests_AccuracyCoin_avalonia.sh --no-screenshots   # Skip screenshots
#   bash run_tests_AccuracyCoin_avalonia.sh --skip 12:1        # Skip P12 item 1 (disables summary run)
#   bash run_tests_AccuracyCoin_avalonia.sh --skip 12:1 --skip 13:7  # Multiple skips
set -u
...
RESULTS_DIR="temp/ac_results_avalonia"
...
if [ $OPT_BUILD -eq 1 ]; then
    echo "=== Building AprNesAvalonia (Debug) ==="
    dotnet build "AprNesAvalonia/AprNesAvalonia.csproj" -c Debug --nologo -v minimal
...
echo "=== Running AccuracyCoin page-by-page (AprNesAvalonia) ==="
echo ""
...
<title>AccuracyCoin Report - AprNesAvalonia</title>
<style>
...
<body>
<h1>AccuracyCoin Report - AprNesAvalonia (.NET 10)</h1>
