#!/bin/bash
# Analog + Audio DSP Multi-Resolution Benchmark (Avalonia / .NET 10)
# Tests AnalogSize 2x/4x/6x/8x with audio DSP enabled (no playback)
# 3-run protocol: discard JIT run, average run 2+3
#
# Usage:
#   bash bench_analog_with_dsp_avalonia.sh --audio-mode <0|1|2> [--no-build]
#
# Examples:
#   bash bench_analog_with_dsp_avalonia.sh --audio-mode 0              # Pure Digital
#   bash bench_analog_with_dsp_avalonia.sh --audio-mode 1              # Authentic (RF+Buzz max cost)
#   bash bench_analog_with_dsp_avalonia.sh --audio-mode 2              # Modern (all effects max)
#   bash bench_analog_with_dsp_avalonia.sh --audio-mode 2 --no-build   # skip build
...
if [[ -z "$AUDIO_MODE" ]] || ! [[ "$AUDIO_MODE" =~ ^[012]$ ]]; then
    ...
    echo ""
    echo "Usage: bash bench_analog_with_dsp_avalonia.sh --audio-mode <0|1|2> [--no-build]"
...
# ── Config ──
PROJ="C:/ai_project/AprNes/AprNesAvalonia/AprNesAvalonia.csproj"
EXE="C:/ai_project/AprNes/AprNesAvalonia/bin/Release/net10.0/AprNesAvalonia.exe"
...
RUN_NUM=0
DATE=$(date +%Y-%m-%d)
OUTFILE="C:/ai_project/AprNes/MD/PerformanceWithAV/Avalonia_Analog_DSP_${MODE_TAG}_Benchmark_${DATE}.md"

echo "============================================================"
echo "  Analog + Audio DSP Benchmark (Avalonia / .NET 10)"
...
if [[ ! -f "$EXE" ]]; then
    ...
    echo "Please build first or check the path."
    exit 1
fi
...
cat > "$OUTFILE" << MDEOF
# Analog + Audio DSP Benchmark — Mode ${AUDIO_MODE} (${MODE_NAME}) [Avalonia / .NET 10]

**日期**: ${DATE}
**測試目的**: 量測 AprNesAvalonia (.NET 10 + TieredPGO) Analog + Audio DSP Mode ${AUDIO_MODE} (${MODE_NAME}) 在不同解析度下的完整管線效能
...
echo ""
echo "=== Summary (Mode ${AUDIO_MODE}: ${MODE_NAME}) [Avalonia / .NET 10] ==="
IDX=0
for SZ in 2 4 6 8; do
    W=$((256 * SZ))
    H=$((210 * SZ))
