...
> Root cause: APU `init()` does not correctly set the initial state. At power-on, $4015 should be $00 (all channels disabled) and $4017 should be written with $00 (mode 0). The current `initAPU()` does not correctly initialize the frame counter state and $4015/$4017 registers.
...
> Root cause: The APU frame counter implementation is missing the following:
> - Writing $80 to $4017 should immediately clock the length counter (mode 1 triggers half-frame instantly)
> - Length table values are incorrect
> - Writing $40/$C0 to $4017 should clear the frame IRQ flag
> - APU clock jitter (even/odd cycle delay) is not handled
> - Frame counter mode 0 length clock timing fires too late
> - Frame IRQ flag is set too early
> - DMC IRQ flag should be cleared when IRQ is disabled
...
|---|---|---|---|---|
| 1 | APU init state ($4015/$4017) | 6 | Low-Medium | All apu_reset failures |
| 2 | APU frame counter timing | 9 | Medium-High | length clock, IRQ flag, jitter |
| 3 | APU length table | 1 | Low | Lookup table value correction |
