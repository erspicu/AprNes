...
.methodology{margin-top:2rem;border-top:1px solid var(--border);padding-top:1rem}
.method-header{display:flex;align-items:center;gap:.5rem;padding:.6rem 1rem;background:var(--surface);border:1px solid var(--border);border-radius:8px;cursor:pointer;user-select:none;font-weight:600;font-size:.9rem;margin-bottom:.75rem}
.method-header:hover{background:var(--surface2)}
.method-header .arrow{font-size:.75rem;color:var(--text-dim)}
.method-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:.85rem}
.method-card{background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:1rem}
.method-card h3{font-size:.85rem;margin-bottom:.5rem;color:var(--accent)}
.method-card p{font-size:.78rem;color:var(--text-dim);margin-bottom:.4rem;line-height:1.55}
.method-card code{background:var(--bg);padding:.1rem .3rem;border-radius:3px;font-size:.72rem}
...
<div class="methodology">
  <div class="method-header" onclick="document.getElementById('method-body').classList.toggle('hidden')">
    <span class="arrow" id="method-arrow">&#9660;</span> Testing Methodology
  </div>
  <div id="method-body">
    <div class="method-grid">
      <div class="method-card">
        <h3>Architecture</h3>
        <p>Headless test runner built into AprNes (<code>TestRunner.cs</code>). Emulator runs at full speed with no audio/rendering overhead. A bash script (<code>run_tests_report.sh</code>) orchestrates 174 test ROMs across 30+ suites, captures screenshots, and generates this report.</p>
      </div>
      <div class="method-card">
        <h3>Test ROM Sources</h3>
        <p>Industry-standard NES hardware verification ROMs by <strong>blargg</strong> and other authors. Covers CPU instructions, PPU timing, APU behavior, mapper IRQ, sprite evaluation, DMA interaction, and controller reading. Each ROM runs real 6502 code to verify hardware-accurate behavior.</p>
      </div>
      <div class="method-card">
        <h3>Result Detection</h3>
        <p><strong>$6000 Protocol:</strong> Modern blargg tests write status to memory address $6000 ($80=running, $81=reset requested, 0=pass, N=fail code). Result text is read from $6004+.</p>
        <p><strong>Screen Detection:</strong> Older tests display results on screen. The runner waits for screen stability (~1.5s unchanged), then scans PPU nametable for "Passed"/"Failed" text or hex status codes ($01=pass, $02+=fail).</p>
      </div>
      <div class="method-card">
        <h3>Automation Features</h3>
        <p><strong>Auto Soft Reset:</strong> Tests requesting reset ($6000=$81) are automatically handled with proper 100ms delay, up to 10 resets per ROM.</p>
        <p><strong>Simulated Input:</strong> Controller tests receive timed button presses via <code>--input</code> parameter.</p>
        <p><strong>Screenshots:</strong> Final frame captured as lossless WebP for visual verification.</p>
      </div>
    </div>
  </div>
</div>
