#!/bin/bash
# run_tests_TriCNES.sh — Run all blargg tests on TriCNES with optional JSON/screenshots/HTML report
#
# Usage:
#   bash run_tests_TriCNES.sh                        # Quick: stdout only
#   bash run_tests_TriCNES.sh --json --screenshots   # Full: JSON + screenshots + HTML report
#   bash run_tests_TriCNES.sh --no-build             # Skip build step
set -u

cd /c/ai_project/AprNes

TRICNES_SRC="ref/TriCNES-main"
EXE="$TRICNES_SRC/bin/Debug/TriCNES.exe"
MSBUILD="C:\\Program Files\\Microsoft Visual Studio\\2022\\Community\\MSBuild\\Current\\Bin\\MSBuild.exe"
ROMBASE="nes-test-roms-master/checked"
REPORT_DIR="report"
SCREENSHOT_DIR="$REPORT_DIR/screenshots-TriCNES"
RESULTS_JSON="$REPORT_DIR/TriCNES_results.json"
HTML_REPORT="$REPORT_DIR/TriCNES_report.html"
...
if [ $OPT_BUILD -eq 1 ]; then
    ...
    WIN_PROJ="$(cygpath -w "$PWD/$TRICNES_SRC/TriCNES.csproj")"
    powershell -NoProfile -Command "& '$MSBUILD' '$WIN_PROJ' /p:Configuration=Debug /t:Rebuild /nologo /v:minimal"
    if [ $? -ne 0 ]; then
        echo "BUILD FAILED"
        exit 1
    fi
...
run_test() {
    ...
    if [ $OPT_SCREENSHOTS -eq 1 ]; then
        ...
        ss_rel="screenshots-TriCNES/${suite}/${rom_base}.webp"
        local ss_png_path="$REPORT_DIR/screenshots-TriCNES/${suite}/${rom_base}.png"
...
" 2>/dev/null && rm -f "$ss_png_path"
    ...
    # JSON output — use Python for reliable JSON escaping
    if [ $OPT_JSON -eq 1 ]; then
        python -c "
import json, sys
obj = {'suite': sys.argv[1], 'rom': sys.argv[2], 'status': sys.argv[3],
       'exit_code': int(sys.argv[4]), 'result_text': sys.argv[5], 'screenshot': sys.argv[6]}
...
" "$suite" "$rom" "$status" "$rc" "$output" "$ss_rel" "$RESULTS_JSON" "$FIRST_JSON"
        FIRST_JSON=0
    fi
}

# ─────────────────────────────────────────────
# Step 2: Run all tests (same 174 ROMs as AprNes)
# ─────────────────────────────────────────────
echo ""
echo "=== Running TriCNES tests ==="

# apu_mixer (4)
for rom in dmc.nes noise.nes square.nes triangle.nes; do
    run_test "apu_mixer" "$rom" "--max-wait 60"
done
...
for rom in 4015_cleared.nes 4017_timing.nes 4017_written.nes irq_flag_cleared.nes len_ctrs_enabled.nes works_immediately.nes; do
    run_test "apu_reset" "$rom" "--max-wait 60"
done

# apu_test merged + singles (9)
run_test "apu_test" "apu_test.nes" "--max-wait 300"
for rom in 1-len_ctr.nes 2-len_table.nes 3-irq_flag.nes 4-jitter.nes 5-len_timing.nes 6-irq_flag_timing.nes 7-dmc_basics.nes 8-dmc_rates.nes; do
    run_test "apu_test/rom_singles" "$rom" "--max-wait 60"
done
...
for rom in 01.len_ctr.nes 02.len_table.nes 03.irq_flag.nes 04.clock_jitter.nes 05.len_timing_mode0.nes 06.len_timing_mode1.nes 07.irq_flag_timing.nes 08.irq_timing.nes 09.reset_timing.nes 10.len_halt_timing.nes 11.len_reload_timing.nes; do
    run_test "blargg_apu_2005.07.30" "$rom" "--max-wait 60"
done

# blargg_nes_cpu_test5 (2)
for rom in cpu.nes official.nes; do
    run_test "blargg_nes_cpu_test5" "$rom" "--max-wait 300"
done
...
for rom in palette_ram.nes power_up_palette.nes sprite_ram.nes vbl_clear_time.nes vram_access.nes; do
    run_test "blargg_ppu_tests_2005.09.15b" "$rom" "--max-wait 60"
done

# branch_timing_tests (3)
for rom in "1.Branch_Basics.nes" "2.Backward_Branch.nes" "3.Forward_Branch.nes"; do
    run_test "branch_timing_tests" "$rom" "--max-wait 60"
done

# cpu_dummy_reads (1)
run_test "cpu_dummy_reads" "cpu_dummy_reads.nes" "--max-wait 60"

# cpu_dummy_writes (2)
for rom in cpu_dummy_writes_oam.nes cpu_dummy_writes_ppumem.nes; do
    run_test "cpu_dummy_writes" "$rom" "--max-wait 60"
done

# cpu_exec_space (2)
for rom in test_cpu_exec_space_ppuio.nes test_cpu_exec_space_apu.nes; do
    run_test "cpu_exec_space" "$rom" "--max-wait 60"
done

# cpu_interrupts_v2 merged + singles (6)
run_test "cpu_interrupts_v2" "cpu_interrupts.nes" "--max-wait 300"
for rom in 1-cli_latency.nes 2-nmi_and_brk.nes 3-nmi_and_irq.nes 4-irq_and_dma.nes 5-branch_delays_irq.nes; do
    run_test "cpu_interrupts_v2/rom_singles" "$rom" "--max-wait 60"
done

# cpu_reset (2)
for rom in registers.nes ram_after_reset.nes; do
    run_test "cpu_reset" "$rom" "--max-wait 60"
done

# cpu_timing_test6 (1)
run_test "cpu_timing_test6" "cpu_timing_test.nes" "--max-wait 300"

# dmc_dma_during_read4 (5)
run_test "dmc_dma_during_read4" "dma_2007_read.nes" "--max-wait 60 --expected-crc 159A7A8F,5E3DF9C4"
run_test "dmc_dma_during_read4" "dma_2007_write.nes" "--max-wait 60"
run_test "dmc_dma_during_read4" "dma_4016_read.nes" "--max-wait 60"
run_test "dmc_dma_during_read4" "double_2007_read.nes" "--max-wait 60 --expected-crc 85CFD627,F018C287,440EF923,E52F41A5"
run_test "dmc_dma_during_read4" "read_write_2007.nes" "--max-wait 60"

# instr_misc merged + singles (5)
run_test "instr_misc" "instr_misc.nes" "--max-wait 300"
for rom in 01-abs_x_wrap.nes 02-branch_wrap.nes 03-dummy_reads.nes 04-dummy_reads_apu.nes; do
    run_test "instr_misc/rom_singles" "$rom" "--max-wait 60"
done

# instr_test-v3 (17)
run_test "instr_test-v3" "all_instrs.nes" "--max-wait 300"
run_test "instr_test-v3" "official_only.nes" "--max-wait 300"
for rom in 01-implied.nes 02-immediate.nes 03-zero_page.nes 04-zp_xy.nes 05-absolute.nes 06-abs_xy.nes 07-ind_x.nes 08-ind_y.nes 09-branches.nes 10-stack.nes 11-jmp_jsr.nes 12-rts.nes 13-rti.nes 14-brk.nes 15-special.nes; do
    run_test "instr_test-v3/rom_singles" "$rom" "--max-wait 60"
done

# instr_test-v5 (18)
run_test "instr_test-v5" "all_instrs.nes" "--max-wait 300"
run_test "instr_test-v5" "official_only.nes" "--max-wait 300"
...
for rom in 01-implied.nes 02-immediate.nes 03-zero_page.nes 04-zp_xy.nes 05-absolute.nes 06-abs_xy.nes 07-ind_x.nes 08-ind_y.nes 09-branches.nes 10-stack.nes 11-special.nes; do
    run_test "nes_instr_test/rom_singles" "$rom" "--max-wait 60"
done

# oam_read (1)
run_test "oam_read" "oam_read.nes" "--max-wait 60"

# ppu_open_bus (1)
run_test "ppu_open_bus" "ppu_open_bus.nes" "--max-wait 60"

# ppu_read_buffer (1)
run_test "ppu_read_buffer" "test_ppu_read_buffer.nes" "--max-wait 60"

# ppu_vbl_nmi merged + singles (11)
run_test "ppu_vbl_nmi" "ppu_vbl_nmi.nes" "--max-wait 300"
for rom in 01-vbl_basics.nes 02-vbl_set_time.nes 03-vbl_clear_time.nes 04-nmi_control.nes 05-nmi_timing.nes 06-suppression.nes 07-nmi_on_timing.nes 08-nmi_off_timing.nes 09-even_odd_frames.nes 10-even_odd_timing.nes; do
    run_test "ppu_vbl_nmi/rom_singles" "$rom" "--max-wait 60"
done

# read_joy3 (4) — test_buttons needs --input for controller simulation; use --pass-on-stable for error counter tests
run_test "read_joy3" "test_buttons.nes" "--max-wait 30 --input A:2.0,B:4.0,Select:6.0,Start:8.0,Up:10.0,Down:12.0,Left:14.0,Right:16.0"
for rom in count_errors.nes count_errors_fast.nes; do
    run_test "read_joy3" "$rom" "--max-wait 60 --pass-on-stable"
done
run_test "read_joy3" "thorough_test.nes" "--max-wait 60"

# sprdma_and_dmc_dma (2)
for rom in sprdma_and_dmc_dma.nes sprdma_and_dmc_dma_512.nes; do
    run_test "sprdma_and_dmc_dma" "$rom" "--max-wait 60"
done
...
for rom in 1.frame_basics.nes 2.vbl_timing.nes 3.even_odd_frames.nes 4.vbl_clear_timing.nes 5.nmi_suppression.nes 6.nmi_disable.nes 7.nmi_timing.nes; do
    run_test "vbl_nmi_timing" "$rom" "--max-wait 60"
done
...
<title>TriCNES Test Report</title>
<style>
...
<div class="footer">Generated by TriCNES Test Runner</div>
<script>
HTML1
