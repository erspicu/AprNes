...
namespace AprNes
{
    ...
    unsafe static public partial class NesCore
    {
        ...
        // SIMD capability flags — evaluated once at startup, cached for hot-path dispatch.
        // Use these instead of calling Avx.IsSupported etc. directly in render loops.
        static readonly bool SimdAvx   = Avx.IsSupported;    // x86: 256-bit AVX (S01/S02 wide clear/fill)
        static readonly bool SimdSse41 = Sse41.IsSupported;  // x86: SSE4.1 (legacy; kept for future use)
        static readonly bool SimdNeon  = AdvSimd.IsSupported; // ARM: NEON (future mapper/audio use)
        ...
        static void Increment2007()
        {
            ...
            if (mapper == 4 || mapper == 9 || mapper == 10) NotifyMapperA12(vram_addr);
        }
        ...
        static void RenderBGTile(int cx)
        {
            ...
            ushort ls = lowshift;
            ushort hs = highshift;
            int scanBase = scanOff + baseX;

            // S03: masked path — only when ShowBgLeft8=false at cx=7 (baseX=0), all 8 pixels masked
            if (!ShowBgLeft8 && baseX < 8)
            {
                var vz = Vector128<int>.Zero;
                var vbg = Vector128.Create(bgColor);
                Vector128.Store(vz, Buffer_BG_array + scanBase);
                Vector128.Store(vz, Buffer_BG_array + scanBase + 4);
                Vector128.Store(vbg, ScreenBuf1x + scanBase);
                Vector128.Store(vbg, ScreenBuf1x + scanBase + 4);
                return;
            }

            // Extract 8 bgPixel values (0-3) via bit reversal:
            // After >>  (8-fineX): bit[7-loc] of lsRev/hsRev = plane0/1 for pixel loc
            int lsRev = (ls >> (8 - fineX)) & 0xFF;
            int hsRev = (hs >> (8 - fineX)) & 0xFF;
            uint p0 = ((uint)(lsRev >> 7) & 1) | (((uint)(hsRev >> 7) & 1) << 1);
            uint p1 = ((uint)(lsRev >> 6) & 1) | (((uint)(hsRev >> 6) & 1) << 1);
            uint p2 = ((uint)(lsRev >> 5) & 1) | (((uint)(hsRev >> 5) & 1) << 1);
            uint p3 = ((uint)(lsRev >> 4) & 1) | (((uint)(hsRev >> 4) & 1) << 1);
            uint p4 = ((uint)(lsRev >> 3) & 1) | (((uint)(hsRev >> 3) & 1) << 1);
            uint p5 = ((uint)(lsRev >> 2) & 1) | (((uint)(hsRev >> 2) & 1) << 1);
            uint p6 = ((uint)(lsRev >> 1) & 1) | (((uint)(hsRev >> 1) & 1) << 1);
            uint p7 = ((uint)(lsRev >> 0) & 1) | (((uint)(hsRev >> 0) & 1) << 1);

            // Write bgPixel indices to Buffer_BG_array (8× int, 2 vector stores)
            Vector128.Store(Vector128.Create((int)p0, (int)p1, (int)p2, (int)p3), Buffer_BG_array + scanBase);
            Vector128.Store(Vector128.Create((int)p4, (int)p5, (int)p6, (int)p7), Buffer_BG_array + scanBase + 4);

            // SIMD palette lookup: pixels [0..splitLoc-1] → palCacheR, [splitLoc..7] → palCacheN
            // splitLoc = 8 - fineX  (when fineX=0: all use palR; fineX=7: only pixel 0 uses palR)
            int splitLoc = 8 - fineX;
            var palR = Vector128.Load(palCacheR);
            var palN = Vector128.Load(palCacheN);

            var idx0 = Vector128.Create(p0, p1, p2, p3);
            var sm0 = Vector128.Create(
                0 < splitLoc ? uint.MaxValue : 0u,
                1 < splitLoc ? uint.MaxValue : 0u,
                2 < splitLoc ? uint.MaxValue : 0u,
                3 < splitLoc ? uint.MaxValue : 0u);
            Vector128.Store(
                Vector128.ConditionalSelect(sm0, Vector128.Shuffle(palR, idx0), Vector128.Shuffle(palN, idx0)),
                ScreenBuf1x + scanBase);

            var idx1 = Vector128.Create(p4, p5, p6, p7);
            var sm1 = Vector128.Create(
                4 < splitLoc ? uint.MaxValue : 0u,
                5 < splitLoc ? uint.MaxValue : 0u,
                6 < splitLoc ? uint.MaxValue : 0u,
                7 < splitLoc ? uint.MaxValue : 0u);
            Vector128.Store(
                Vector128.ConditionalSelect(sm1, Vector128.Shuffle(palR, idx1), Vector128.Shuffle(palN, idx1)),
                ScreenBuf1x + scanBase + 4);
        }
        ...
        static void ppu_rendering_tick(int cx)
        {
            if (cx < 256 || (cx >= 320 && cx < 336))
            {
                ...
                if (phase == 0) {
                    ...
                    if (mapper == 4 || mapper == 9 || mapper == 10) NotifyMapperA12(ioaddr);  // CHR low addr
                } else if (phase == 5) {
                    lowTile = chrBankPtrs[(ioaddr >> 10) & 7][ioaddr & 0x3FF];
                } else if (phase == 6) {
                    ioaddr = BgPatternTableAddr | (NTVal << 4) | ((vram_addr >> 12) & 7) | 8;
                    if (mapper == 9 || mapper == 10) NotifyMapperA12(ioaddr);  // MMC2/MMC4: CHR high triggers latch
                } else {
            ...
            if ((mapper == 4 || mapper == 9 || mapper == 10) && cx == 336)
        ...
        static void ppu_step_new()
        {
            ...
            if (scanline < 240 || scanline == 261)
            {
                ...
                if (scanline >= 0 && scanline < 240)
                {
                    ...
                    {
                        int scanOff = scanline << 8;
                        // S01: zero Buffer_BG_array for this scanline (256×int = 1024 bytes)
                        // AVX path: 32 bytes/store × 32; fallback: platform-agnostic Vector128 × 64
                        if (SimdAvx)
                        {
                            var vzero = Vector256<int>.Zero;
                            int* bgp = Buffer_BG_array + scanOff;
                            for (int* bge = bgp + 256; bgp < bge; bgp += 8)
                                Avx.Store(bgp, vzero);
                        }
                        else
                        {
                            ...
                            int* bgp = Buffer_BG_array + scanOff;
                            for (int* bge = bgp + 256; bgp < bge; bgp += 4)
                                Vector128.Store(vzero, bgp);
                        }
                        if (!ShowBackGround)
                        {
                            // S02: fill ScreenBuf1x with bgColor (256×uint = 1024 bytes)
                            uint bgColor = NesColors[ppu_ram[0x3f00] & 0x3f];
                            if (SimdAvx)
                            {
                                var vcolor = Vector256.Create(bgColor);
                                uint* sp = ScreenBuf1x + scanOff;
                                for (uint* se = sp + 256; sp < se; sp += 8)
                                    Avx.Store(sp, vcolor);
                            }
                            else
                            {
                                var vcolor = Vector128.Create(bgColor);
                                uint* sp = ScreenBuf1x + scanOff;
                                for (uint* se = sp + 256; sp < se; sp += 4)
                                    Vector128.Store(vcolor, sp);
                            }
                        }
                        PrecomputeOverflow();
                    }
        ...
        // Sprite Pass 3 SIMD composite (platform-agnostic Vector128, 4 pixels/cycle)
        // writeMask = sprSet[x]!=0  AND  (!ShowBG OR BG==0 OR priority==0)
        // result    = writeMask ? sprColor[x] : ScreenBuf1x[scanOff+x]
        // Works on x86 (SSE2/NEON auto-selected by JIT) and ARM NEON.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void CompositeSpritesSimd(int scanOff, byte* sprSet, byte* sprPriority, uint* sprColor)
        {
            ...
            for (int x = 0; x < 256; x += 4)
            {
                ...
                var sprSetExp  = Vector128.Create((int)sprSet[x], (int)sprSet[x+1], (int)sprSet[x+2], (int)sprSet[x+3]);
                var hasSprMask = Vector128.GreaterThan(sprSetExp, zero);

                // Fast-skip: none of these 4 pixels has a sprite
                if (hasSprMask.ExtractMostSignificantBits() == 0) continue;

                // Zero-extend 4 sprPriority bytes → front-priority mask (priority==0 → front)
                var priExp    = Vector128.Create((int)sprPriority[x], (int)sprPriority[x+1], (int)sprPriority[x+2], (int)sprPriority[x+3]);
                var frontMask = Vector128.Equals(priExp, zero);

                // BG-transparent mask
                var bgVec        = Vector128.Load(Buffer_BG_array + scanOff + x);
                var bgTranspMask = Vector128.Equals(bgVec, zero);

                // condMask: !ShowBG → always write; else write if BG transparent or front-priority
                var condMask = ShowBackGround
                    ? Vector128.BitwiseOr(bgTranspMask, frontMask)
                    : allOnes;

                // writeMask = hasSpr & cond
                var writeMask = Vector128.BitwiseAnd(hasSprMask, condMask);

                // Blend: select sprColor where writeMask, else keep screen
                var sprColorVec = Vector128.Load((int*)(sprColor + x));
                var screenVec   = Vector128.Load((int*)(ScreenBuf1x + scanOff + x));
                var result      = Vector128.ConditionalSelect(writeMask, sprColorVec, screenVec);

                Vector128.Store(result, (int*)(ScreenBuf1x + scanOff + x));
            }
        }
