...
namespace AprNes
{
    unsafe public partial class NesCore
    {
        ...
        static void DoHLT()
        {
            ...
            else if (operationCycle  < 5) { addressBus = 0xFFFE; CpuRead(addressBus); } // case 3 & 4
            else if (operationCycle == 5) { addressBus = 0xFFFF; CpuRead(addressBus); }
            else                          { addressBus = 0xFFFF; CpuRead(addressBus); operationCycle = 5; } // loop
        }
        ...
        static void Op_00() {
            if (operationCycle == 1) {
                if (!doBRK) { CpuRead(addressBus); } else { GetImmediate(); }
            }
            else if (operationCycle == 2) {
                ...
                if (!doReset) { byte pushed = (byte)(GetFlag() | 0x20 | (doBRK ? 0x10 : 0x00)); StackPush(pushed); }
                else { CpuRead((ushort)(0x100 | r_SP)); r_SP--; }
        ...
        static void Op_24() {
            if (operationCycle == 1) GetAddressZeroPage();
            else { dl = CpuRead(addressBus); flagZ = (byte)(((r_A & dl) == 0) ? 1 : 0); flagN = (byte)((dl & 0x80) >> 7); flagV = (byte)((dl & 0x40) >> 6); CompleteOperation(); }
        }
        static void Op_2C() {
            if (operationCycle < 3) GetAddressAbsolute();
            else { dl = CpuRead(addressBus); flagZ = (byte)(((r_A & dl) == 0) ? 1 : 0); flagN = (byte)((dl & 0x80) >> 7); flagV = (byte)((dl & 0x40) >> 6); CompleteOperation(); }
        ...
        static void Op_6C() {
            ...
            else { dl = CpuRead((ushort)((addressBus & 0xFF00) | (byte)(addressBus + 1))); r_PC = (ushort)((dl << 8) | specialBus); CompleteOperation(); }
        }

        // === JSR ===
        static void Op_20() {
            if      (operationCycle == 1) { addressBus = r_PC; dl = CpuRead(addressBus); r_PC++; }
            else if (operationCycle == 2) { addressBus = (ushort)(0x100 | r_SP); specialBus = dl; CpuRead(addressBus); }
            else if (operationCycle == 3) { CpuWrite(addressBus, (byte)(r_PC >> 8)); addressBus = (ushort)((byte)(addressBus - 1) | 0x100); }
            else if (operationCycle == 4) { CpuWrite(addressBus, (byte)r_PC); addressBus = (ushort)((byte)(addressBus - 1) | 0x100); r_SP = (byte)addressBus; }
            else { r_PC = (ushort)((CpuRead(r_PC) << 8) | specialBus); CompleteOperation(); }
        }

        // === RTS ===
        static void Op_60() {
            if      (operationCycle == 1) { GetImmediate(); }
            else if (operationCycle == 2) { addressBus = (ushort)(0x100 | r_SP); CpuRead(addressBus); addressBus = (ushort)((byte)(addressBus + 1) | 0x100); }
            else if (operationCycle == 3) { dl = CpuRead(addressBus); r_PC = (ushort)((r_PC & 0xFF00) | dl); addressBus = (ushort)((byte)(addressBus + 1) | 0x100); }
            else if (operationCycle == 4) { dl = CpuRead(addressBus); r_PC = (ushort)((r_PC & 0xFF) | (dl << 8)); }
            else { r_SP = (byte)addressBus; GetImmediate(); CompleteOperation(); }
        }
        ...
        static void Op_40() {
            ...
            else if (operationCycle == 3) { byte status = CpuRead(addressBus); SetFlag(status); addressBus = (ushort)((byte)(addressBus + 1) | 0x100); }
            else if (operationCycle == 4) { dl = CpuRead(addressBus); r_PC = (ushort)((r_PC & 0xFF00) | dl); addressBus = (ushort)((byte)(addressBus + 1) | 0x100); }
            else { dl = CpuRead(addressBus); r_PC = (ushort)((r_PC & 0xFF) | (dl << 8)); r_SP = (byte)addressBus; CompleteOperation(); }
        }
        ...
        static void Op_BB() {
            if (operationCycle < 4) GetAddressAbsOffY(true);
            else { dl = CpuRead(addressBus); r_A = (byte)(dl & r_SP); r_X = r_A; r_SP = r_A; SetNZ(r_A); CompleteOperation(); }
        }
        ...
        static void Op_93() {
            if (operationCycle < 5) GetAddressIndOffY(false);
                ...
                if ((temporaryAddress & 0xFF00) != (addressBus & 0xFF00)) addressBus = (ushort)((byte)addressBus | (((addressBus >> 8) & r_X) << 8));
                if (ignoreH) H = 0xFF;
                CpuWrite(addressBus, (byte)(r_A & (r_X | 0xF5) & H)); CompleteOperation();
            }
        }
