#define illegal
using System.Windows.Forms;
using System;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

namespace AprNes
{
    unsafe public partial class NesCoreSpeed
    {
        //table port from  https://github.com/bfirsh/jsnes/blob/master/source/cpu.js
        static byte[] OPCODE_CYCLE_TABLE = new byte[]{
    /*0x00*/ 7,6,2,8,3,3,5,5,3,2,2,2,4,4,6,6,
    /*0x10*/ 2,5,2,8,4,4,6,6,2,4,2,7,4,4,7,7,
    /*0x20*/ 6,6,2,8,3,3,5,5,4,2,2,2,4,4,6,6,
    /*0x30*/ 2,5,2,8,4,4,6,6,2,4,2,7,4,4,7,7,
    /*0x40*/ 6,6,2,8,3,3,5,5,3,2,2,2,3,4,6,6,
    /*0x50*/ 2,5,2,8,4,4,6,6,2,4,2,7,4,4,7,7,
    /*0x60*/ 6,6,2,8,3,3,5,5,4,2,2,2,5,4,6,6,
    /*0x70*/ 2,5,2,8,4,4,6,6,2,4,2,7,4,4,7,7,
    /*0x80*/ 2,6,2,6,3,3,3,3,2,2,2,2,4,4,4,4,
    /*0x90*/ 2,6,2,6,4,4,4,4,2,5,2,5,5,5,5,5,
    /*0xA0*/ 2,6,2,6,3,3,3,3,2,2,2,2,4,4,4,4,
    /*0xB0*/ 2,5,2,5,4,4,4,4,2,4,2,4,4,4,4,4,
    /*0xC0*/ 2,6,2,8,3,3,5,5,2,2,2,2,4,4,6,6,
    /*0xD0*/ 2,5,2,8,4,4,6,6,2,4,2,7,4,4,7,7,
    /*0xE0*/ 2,6,2,8,3,3,5,5,2,2,2,2,4,4,6,6,
    /*0xF0*/ 2,5,2,8,4,4,6,6,2,4,2,7,4,4,7,7
    };
        static byte r_A_S = 0, r_X_S = 0, r_Y_S = 0, r_SP_S = 0xFD, flagN_S = 0, flagV_S = 0, flagD_S = 0, flagI_S = 1, flagZ_S = 0, flagC_S = 0;
        static ushort r_PC_S = 0;
        static byte opcode_S;

        static int cpu_cycles_S = 0, Interrupt_cycle_S = 0;
        static bool nmi_pending_S = false;
        static bool nmi_delayed_S = false; // NMI edge detected on last CPU cycle, delay 1 instruction
        public static bool irq_pending_S = false; // IRQ should fire before next instruction

        static bool softreset_S = false;
        public static void SoftReset_S()
        {
            softreset_S = true;
        }

        static void init_cpu_S()
        {
            r_A_S = 0; r_X_S = 0; r_Y_S = 0; r_SP_S = 0xFD;
            flagN_S = 0; flagV_S = 0; flagD_S = 0; flagI_S = 1; flagZ_S = 0; flagC_S = 0;
            r_PC_S = (ushort)(Mem_r_S(0xFFFC) | (Mem_r_S(0xFFFD) << 8));
            nmi_pending_S = false; nmi_delayed_S = false; irq_pending_S = false;
            Interrupt_cycle_S = 0;
        }

        // SP-23: Direct PRG ROM fetch for PC-relative reads — bypasses 65536-entry dispatch table.
        // PC is always in $8000-$FFFF during normal execution; prgBankPtrs_S covers this range.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static byte Fetch_S()
        {
            ushort pc = r_PC_S++;
            return prgBankPtrs_S[(pc >> 13) & 7][pc & 0x1FFF];
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static byte GetFlag_S()
        {
            return (byte)((flagN_S << 7) | (flagV_S << 6) | (flagD_S << 3) | (flagI_S << 2) | (flagZ_S << 1) | flagC_S);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void SetFlag_S(byte flag)
        {
            flagN_S = (byte)((flag & 0x80) >> 7);
            flagV_S = (byte)((flag & 0x40) >> 6);
            flagD_S = (byte)((flag & 0x8) >> 3);
            flagI_S = (byte)((flag & 0x4) >> 2);
            flagZ_S = (byte)((flag & 0x2) >> 1);
            flagC_S = (byte)(flag & 0x1);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void NMIInterrupt_S()
        {
            byte pushed_flags = (byte)(GetFlag_S() | 0x20);
            Mem_w_S((ushort)(0x100 | r_SP_S--), (byte)(r_PC_S >> 8));
            Mem_w_S((ushort)(0x100 | r_SP_S--), (byte)r_PC_S);
            Mem_w_S((ushort)(0x100 | r_SP_S--), pushed_flags);
            r_PC_S = (ushort)(Mem_r_S(0xfffa) | (Mem_r_S(0xfffb) << 8));
            flagI_S = 1;
            Interrupt_cycle_S = 7;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void ResetInterrupt_S()
        {
            Console.WriteLine("soft reset !");
            r_SP_S -= 3;
            r_PC_S = (ushort)(Mem_r_S(0xfffc) | (Mem_r_S(0xfffd) << 8));
            flagI_S = 1;
            nmi_pending_S = false;
            nmi_delayed_S = false;
            irq_pending_S = false;
            Interrupt_cycle_S = 7;
            apuSoftReset_S();
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void IRQInterrupt_S()
        {
            Mem_w_S((ushort)(0x100 | r_SP_S--), (byte)(r_PC_S >> 8));
            Mem_w_S((ushort)(0x100 | r_SP_S--), (byte)r_PC_S);
            Mem_w_S((ushort)(0x100 | r_SP_S--), (byte)(GetFlag_S() | 0x20));
            r_PC_S = (ushort)(Mem_r_S(0xfffe) | (Mem_r_S(0xffff) << 8));
            flagI_S = 1;
            Interrupt_cycle_S = 7;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void cpu_step_S()
        {
            ushort ushort1, ushort2, ushort3;
            byte byte1, byte2, byte3;
            int int1;

            if (softreset_S)
            {
                ResetInterrupt_S();
                softreset_S = false;
            }

            // NMI / IRQ check before opcode fetch
            if (nmi_pending_S)
            {
                nmi_pending_S = false;
                NMIInterrupt_S();
            }
            else if (irq_pending_S && flagI_S == 0)
            {
                irq_pending_S = false;
                IRQInterrupt_S();
            }

            byte prevFlagI = flagI_S;
            opcode_S = Fetch_S();
            cpu_cycles_S = OPCODE_CYCLE_TABLE[opcode_S];

            cpu_cycles_S += Interrupt_cycle_S;
            Interrupt_cycle_S = 0;

            //參考了 mynes 去修正與debug許多錯誤 http://sourceforge.net/projects/mynes
            switch (opcode_S)
            {
                case 0x69: //ADC  Immediate  fix
                    byte1 = Fetch_S();
                    int1 = byte1 + r_A_S + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x65: //ADC  Zero Page
                    byte1 = NES_MEM_S[Fetch_S()];
                    int1 = byte1 + r_A_S + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x75://ADC Zero Page,X
                    byte1 = NES_MEM_S[(byte)(Fetch_S() + r_X_S)];
                    int1 = byte1 + r_A_S + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x6D: //ADC Absolute //fix
                    byte1 = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8)));
                    int1 = byte1 + r_A_S + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x7D: //ADC  Absolute,X
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = Mem_r_S(ushort2);
                    int1 = byte1 + r_A_S + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x79: //ADC  Absolute,Y
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = Mem_r_S(ushort2);
                    int1 = byte1 + r_A_S + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x61: //ADC (Indirect,X)
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = Mem_r_S((ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8)));
                    int1 = byte1 + r_A_S + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x71: //ADC (Indirect),Y
                    byte2 = Fetch_S();
                    ushort1 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = Mem_r_S(ushort2);
                    int1 = byte1 + r_A_S + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                //--- AND BEGIN
                case 0x29: //AND  Immediate
                    int1 = Fetch_S() & r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x25: //AND  Zero Page
                    int1 = NES_MEM_S[Fetch_S()] & r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x35://AND Zero Page,X
                    int1 = NES_MEM_S[(byte)(Fetch_S() + r_X_S)] & r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x2D: //AND Absolute
                    int1 = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8))) & r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x3D: //AND  Absolute,X
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    int1 = Mem_r_S(ushort2) & r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x39: //AND  Absolute,Y
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    int1 = Mem_r_S(ushort2) & r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x21: //AND (Indirect,X)
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    ushort1 = (ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8));
                    int1 = Mem_r_S(ushort1) & r_A_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x31: //AND (Indirect),Y
                    byte1 = Fetch_S();
                    ushort1 = (ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    int1 = Mem_r_S(ushort2) & r_A_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;
                //--- AND END

                case 0x0A://ASL acc
                    if ((r_A_S & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S <<= 1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x06://ASL zp
                    byte2 = Fetch_S();
                    byte1 = NES_MEM_S[byte2];
                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    if (byte1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    NES_MEM_S[byte2] = byte1;
                    break;

                case 0x16://ASL zp,x
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = NES_MEM_S[byte2];
                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    if (byte1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    NES_MEM_S[byte2] = byte1;
                    break;

                case 0x0E://ASL abs
                    ushort2 = (ushort)((Fetch_S() | (Fetch_S() << 8)));
                    byte1 = Mem_r_S(ushort2);
                    Mem_w_S(ushort2, byte1);//dummy write fixed
                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    if (byte1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    Mem_w_S(ushort2, byte1);
                    break;

                case 0x1E://ASL abs,x
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF))); // dummy read wrong page
                    byte1 = Mem_r_S(ushort2);
                    Mem_w_S(ushort2, byte1);//dummy write fixed
                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    if (byte1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    Mem_w_S(ushort2, byte1);
                    break;

                case 0x90://BCC branch ,cycle fixed 2017.01.18
                    byte1 = Fetch_S();
                    if (flagC_S == 0)
                    {
                        cpu_cycles_S += 1;
                        byte2 = (byte)(r_PC_S + byte1);
                        byte3 = (byte)(r_PC_S >> 8);
                        if (byte1 >= 0x80)
                        {
                            if (byte2 >= byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3--;
                            }
                        }
                        else
                        {
                            if (byte2 < byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3++;
                            }

                        }
                        r_PC_S = (ushort)(byte2 | (byte3 << 8));
                    }
                    break;

                case 0xB0://BCS branch ,cycle fixed 2017.01.18
                    byte1 = Fetch_S();
                    if (flagC_S == 1)
                    {
                        cpu_cycles_S += 1;
                        byte2 = (byte)(r_PC_S + byte1);
                        byte3 = (byte)(r_PC_S >> 8);
                        if (byte1 >= 0x80)
                        {
                            if (byte2 >= byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3--;
                            }
                        }
                        else
                        {
                            if (byte2 < byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3++;
                            }

                        }
                        r_PC_S = (ushort)(byte2 | (byte3 << 8));
                    }
                    break;

                case 0xF0://BEQ branch ,cycle fixed 2017.01.18
                    byte1 = Fetch_S();
                    if (flagZ_S == 1)
                    {
                        cpu_cycles_S += 1;
                        byte2 = (byte)(r_PC_S + byte1);
                        byte3 = (byte)(r_PC_S >> 8);
                        if (byte1 >= 0x80)
                        {
                            if (byte2 >= byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3--;
                            }
                        }
                        else
                        {
                            if (byte2 < byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3++;
                            }

                        }
                        r_PC_S = (ushort)(byte2 | (byte3 << 8));
                    }
                    break;

                case 0x24://BIT zp fix
                    byte1 = NES_MEM_S[Fetch_S()];
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if ((byte1 & 0x40) > 0) flagV_S = 1; else flagV_S = 0;
                    if ((byte1 & r_A_S) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x2C://BIT abs //FIX
                    ushort t1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte1 = Mem_r_S(t1);
                    if ((byte1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if ((byte1 & 0x40) > 0) flagV_S = 1; else flagV_S = 0;
                    if ((byte1 & r_A_S) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x30://BMI branch ,cycle fixed 2017.01.18
                    byte1 = Fetch_S();
                    if (flagN_S == 1)
                    {
                        cpu_cycles_S += 1;
                        byte2 = (byte)(r_PC_S + byte1);
                        byte3 = (byte)(r_PC_S >> 8);
                        if (byte1 >= 0x80)
                        {
                            if (byte2 >= byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3--;
                            }
                        }
                        else
                        {
                            if (byte2 < byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3++;
                            }

                        }
                        r_PC_S = (ushort)(byte2 | (byte3 << 8));
                    }
                    break;

                case 0xD0://BNE branch ,cycle fixed 2017.01.18
                    byte1 = Fetch_S();
                    if (flagZ_S == 0)
                    {
                        cpu_cycles_S += 1;
                        byte2 = (byte)(r_PC_S + byte1);
                        byte3 = (byte)(r_PC_S >> 8);
                        if (byte1 >= 0x80)
                        {
                            if (byte2 >= byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3--;
                            }
                        }
                        else
                        {
                            if (byte2 < byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3++;
                            }

                        }
                        r_PC_S = (ushort)(byte2 | (byte3 << 8));
                    }
                    break;

                case 0x10://BPL branch ,cycle fixed 2017.01.18
                    byte1 = Fetch_S();
                    if (flagN_S == 0)
                    {
                        cpu_cycles_S += 1;
                        byte2 = (byte)(r_PC_S + byte1);
                        byte3 = (byte)(r_PC_S >> 8);
                        if (byte1 >= 0x80)
                        {
                            if (byte2 >= byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3--;
                            }
                        }
                        else
                        {
                            if (byte2 < byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3++;
                            }
                        }
                        r_PC_S = (ushort)(byte2 | (byte3 << 8));
                    }
                    break;

                case 00://BRK
                    Mem_r_S(r_PC_S);//dummy read fixed 2017.01.19
                    r_PC_S++;
                    Mem_w_S((ushort)(r_SP_S-- | 0x100), (byte)(r_PC_S >> 8));
                    Mem_w_S((ushort)(r_SP_S-- | 0x100), (byte)r_PC_S); //fixed 2017.01.20
                    Mem_w_S((ushort)(r_SP_S-- | 0x100), (byte)(GetFlag_S() | 0x30));
                    flagI_S = 1;
                    r_PC_S = (ushort)(Mem_r_S(0xfffe) | (Mem_r_S((ushort)(0xffff)) << 8));
                    break;

                case 0x50://BVC branch ,cycle fixed 2017.01.18
                    byte1 = Fetch_S();
                    if (flagV_S == 0)
                    {
                        cpu_cycles_S += 1;
                        byte2 = (byte)(r_PC_S + byte1);
                        byte3 = (byte)(r_PC_S >> 8);
                        if (byte1 >= 0x80)
                        {
                            if (byte2 >= byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3--;
                            }
                        }
                        else
                        {
                            if (byte2 < byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3++;
                            }

                        }
                        r_PC_S = (ushort)(byte2 | (byte3 << 8));
                    }
                    break;

                case 0x70://BVS branch ,cycle fixed 2017.01.18
                    byte1 = Fetch_S();
                    if (flagV_S == 1)
                    {
                        cpu_cycles_S += 1;
                        byte2 = (byte)(r_PC_S + byte1);
                        byte3 = (byte)(r_PC_S >> 8);
                        if (byte1 >= 0x80)
                        {
                            if (byte2 >= byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3--;
                            }
                        }
                        else
                        {
                            if (byte2 < byte1)
                            {
                                cpu_cycles_S += 1;
                                byte3++;
                            }

                        }
                        r_PC_S = (ushort)(byte2 | (byte3 << 8));
                    }
                    break;

                case 0x18: flagC_S = 0; break;//CLC
                case 0xD8: flagD_S = 0; break;//CLD

                case 0x58: Mem_r_S(r_PC_S); flagI_S = 0; break;//CLI

                case 0xB8: flagV_S = 0; break;//CLV

                //--- CMP BEGIN
                case 0xC9: //CMP  Immediate
                    byte1 = Fetch_S();
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_A_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xC5: //CMP  Zero Page
                    byte1 = NES_MEM_S[Fetch_S()];
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_A_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xD5://CMP Zero Page,X
                    byte1 = NES_MEM_S[(byte)(Fetch_S() + r_X_S)];
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_A_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xCD: //CMP Absolute
                    byte1 = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8)));
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_A_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xDD: //CMP  Absolute,X
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = Mem_r_S(ushort2);
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_A_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xD9: //CMP  Absolute,Y
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = Mem_r_S(ushort2);
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_A_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xC1: //CMP (Indirect,X)  fix
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    ushort1 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    byte1 = Mem_r_S(ushort1);
                    int1 = r_A_S - byte1;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_A_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xD1: //CMP (Indirect),Y
                    byte2 = Fetch_S();
                    ushort1 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = Mem_r_S(ushort2);
                    int1 = r_A_S - byte1;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_A_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;
                //--- CMP END

                case 0xE0: //CPX  Immediate
                    byte1 = Fetch_S();
                    int1 = r_X_S - byte1;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_X_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xE4: //CPX  Zero Page
                    byte1 = NES_MEM_S[Fetch_S()];
                    int1 = r_X_S - byte1;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_X_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xEC: //CPX Absolute
                    byte1 = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8)));
                    int1 = r_X_S - byte1;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_X_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                //-- CPY BEGIN
                case 0xC0: //CPY  Immediate
                    byte1 = Fetch_S();
                    int1 = r_Y_S - byte1;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_Y_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xC4: //CPY  Zero Page
                    byte1 = NES_MEM_S[Fetch_S()];
                    int1 = r_Y_S - byte1;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_Y_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xCC: //CPY Absolute
                    byte1 = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8)));
                    int1 = r_Y_S - byte1;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (r_Y_S >= byte1) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;
                //-- CPY END

                case 0xC6://DEC zp
                    byte1 = Fetch_S();
                    byte2 = NES_MEM_S[byte1];
                    NES_MEM_S[byte1] = --byte2;
                    if ((byte2 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (byte2 == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xD6://DEC zp,x
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    byte2 = NES_MEM_S[byte1];
                    NES_MEM_S[byte1] = --byte2;
                    if ((byte2 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (byte2 == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xCE://DEC abs
                    ushort1 = (ushort)(Fetch_S() | Fetch_S() << 8);
                    byte1 = Mem_r_S(ushort1);
                    Mem_w_S(ushort1, byte1);//dummy write fixed
                    Mem_w_S(ushort1, --byte1);
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (byte1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xDE://DEC abs,x
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_X_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read wrong page
                    byte1 = Mem_r_S(ushort1);
                    Mem_w_S(ushort1, byte1);//dummy write fixed
                    Mem_w_S(ushort1, --byte1);
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (byte1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xCA://DEX
                    if ((--r_X_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x88://DEY //fix
                    if ((--r_Y_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_Y_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                //--- EOR BEGIN
                case 0x49: //EOR  Immediate
                    int1 = Fetch_S() ^ r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x45: //EOR  Zero Page
                    int1 = NES_MEM_S[Fetch_S()] ^ r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x55://EOR Zero Page,X
                    int1 = NES_MEM_S[(byte)(Fetch_S() + r_X_S)] ^ r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x4D: //EOR Absolute
                    int1 = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8))) ^ r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x5D: //EOR  Absolute,X
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    int1 = Mem_r_S(ushort2) ^ r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x59: //EOR  Absolute,Y
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    int1 = Mem_r_S(ushort2) ^ r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x41: //EOR (Indirect,X)
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    int1 = Mem_r_S((ushort)((NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8)))) ^ r_A_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x51: //EOR (Indirect),Y
                    byte1 = Fetch_S();
                    ushort1 = (ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    int1 = Mem_r_S(ushort2) ^ r_A_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;
                //--- EOR END

                case 0xE6://INC zp
                    byte1 = Fetch_S();
                    byte2 = NES_MEM_S[byte1];
                    NES_MEM_S[byte1] = ++byte2;
                    if ((byte2 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (byte2 == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xF6://INC zp,x
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    byte2 = NES_MEM_S[byte1];
                    NES_MEM_S[byte1] = ++byte2;
                    if ((byte2 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (byte2 == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xEE://INC abs
                    ushort1 = (ushort)(Fetch_S() | Fetch_S() << 8);
                    byte2 = Mem_r_S(ushort1);
                    Mem_w_S(ushort1, byte2);//dummy write fixed 2017.0119
                    Mem_w_S(ushort1, ++byte2);
                    if ((byte2 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (byte2 == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xFE://INC abs,x
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_X_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read wrong page
                    byte2 = Mem_r_S(ushort1);
                    Mem_w_S(ushort1, byte2);//dummy write fixed 2017.0119
                    Mem_w_S(ushort1, ++byte2);
                    if ((byte2 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (byte2 == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xE8://INX
                    if ((++r_X_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xC8://INY
                    if ((++r_Y_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_Y_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x4C://JMP abs
                    r_PC_S = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    break;

                case 0x6C://JMP indirect
                    byte1 = Fetch_S();
                    byte2 = Fetch_S();
                    r_PC_S = (ushort)(Mem_r_S((ushort)((byte1++) | (byte2 << 8))) | (Mem_r_S((ushort)(byte1 | (byte2 << 8))) << 8));
                    break;

                case 0x20://JSR abs
                    ushort1 = (ushort)(r_PC_S + 1);
                    Mem_w_S((ushort)(r_SP_S-- | 0x100), (byte)(ushort1 >> 8));
                    Mem_w_S((ushort)(r_SP_S-- | 0x100), (byte)ushort1);
                    r_PC_S = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    break;

                case 0xA9://LDA imm
                    r_A_S = Fetch_S();
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xA5://LDA zp
                    r_A_S = NES_MEM_S[Fetch_S()];
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xB5://LDA zp,x
                    r_A_S = NES_MEM_S[(byte)(Fetch_S() + r_X_S)];
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xAD://LDA abs
                    r_A_S = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8)));
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xBD://LDA abs,x
                    byte1 = Fetch_S(); //low
                    byte2 = Fetch_S(); //heigh
                    ushort1 = (ushort)(byte1 | (byte2 << 8));
                    byte1 += r_X_S;
                    //
                    r_A_S = Mem_r_S((ushort)(byte1 | (byte2 << 8)));
                    //
                    if (byte1 < r_X_S) r_A_S = Mem_r_S((ushort)(byte1 | ((++byte2) << 8)));//dummy read fiexed 2017.01.17
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((ushort1 & 0xff00) != ((ushort1 + r_X_S) & 0xff00)) cpu_cycles_S++;
                    break;

                case 0xB9://LDA abs,y
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    r_A_S = Mem_r_S(ushort2);
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xA1://LDA (indirect,x)
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    r_A_S = Mem_r_S((ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8)));
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xB1://LDA (indirect),y
                    byte3 = Fetch_S();
                    byte1 = NES_MEM_S[byte3++];
                    byte2 = NES_MEM_S[byte3];
                    ushort1 = (ushort)(byte1 | (byte2 << 8));
                    byte1 += r_Y_S;
                    r_A_S = Mem_r_S((ushort)(byte1 | (byte2 << 8)));
                    //
                    if (byte1 < r_Y_S) r_A_S = Mem_r_S((ushort)(byte1 | ((++byte2) << 8)));//dummy read fiexed 2017.01.17
                    //
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((ushort1 & 0xff00) != ((ushort1 + r_Y_S) & 0xff00)) cpu_cycles_S++;
                    break;

                case 0xA2://LDX imm
                    r_X_S = Fetch_S();
                    if ((r_X_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xA6://LDX zp
                    r_X_S = NES_MEM_S[Fetch_S()];
                    if ((r_X_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xB6://LDX zp,y
                    r_X_S = NES_MEM_S[(byte)(Fetch_S() + r_Y_S)];
                    if ((r_X_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xAE://LDX abs
                    r_X_S = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8)));
                    if ((r_X_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xBE://LDX abs,y
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    r_X_S = Mem_r_S(ushort2);
                    if ((r_X_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xA0://LDY imm
                    r_Y_S = Fetch_S();
                    if ((r_Y_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_Y_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xA4://LDY zp
                    r_Y_S = NES_MEM_S[Fetch_S()];
                    if ((r_Y_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_Y_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xB4://LDY zp,x
                    r_Y_S = NES_MEM_S[(byte)(Fetch_S() + r_X_S)];
                    if ((r_Y_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_Y_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xAC://LDY abs
                    r_Y_S = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8)));
                    if ((r_Y_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_Y_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xBC://LDY abs,x
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    r_Y_S = Mem_r_S(ushort2);
                    if ((r_Y_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_Y_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                //----- LSR begin
                case 0x4A://LSR acc
                    if ((r_A_S & 0x01) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S >>= 1;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x46://LSR zp fix
                    byte2 = Fetch_S();
                    byte1 = NES_MEM_S[byte2];
                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    if ((byte1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    NES_MEM_S[byte2] = byte1;
                    break;

                case 0x56://LSR zp,x
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = NES_MEM_S[byte2];
                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    if ((byte1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    NES_MEM_S[byte2] = byte1;
                    break;

                case 0x4E://LSR abs fix
                    ushort2 = (ushort)(((Fetch_S() << 0) | (Fetch_S() << 8)));
                    byte1 = Mem_r_S(ushort2);
                    Mem_w_S(ushort2, byte1);//dummy write fixed
                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    if ((byte1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    Mem_w_S(ushort2, byte1);
                    break;

                case 0x5E://LSR abs,x
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF))); // dummy read wrong page
                    byte1 = Mem_r_S(ushort2);
                    Mem_w_S(ushort2, byte1);//dummy write fixed
                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    if ((byte1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((byte1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    Mem_w_S(ushort2, byte1);
                    break;
                //---- LSR END

                case 0xEA: break;//NOP

                //--- ORA BEGIN
                case 0x09: //ORA  Immediate
                    int1 = Fetch_S() | r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x05: //ORA  Zero Page
                    int1 = NES_MEM_S[Fetch_S()] | r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x15://ORA Zero Page,X
                    int1 = NES_MEM_S[(byte)(Fetch_S() + r_X_S)] | r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x0D: //ORA Absolute
                    int1 = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8))) | r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x1D: //ORA  Absolute,X  fix
                    byte1 = Fetch_S();
                    byte2 = Fetch_S();
                    byte1 += r_X_S;
                    ushort2 = Mem_r_S((ushort)(byte1 | (byte2 << 8)));
                    if (byte1 < r_X_S)
                    {
                        byte2++;
                        ushort2 = Mem_r_S((ushort)(byte1 | ((byte2) << 8)));
                        cpu_cycles_S++;
                    }
                    int1 = ushort2 | r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x19: //ORA  Absolute,Y
                    byte1 = Fetch_S();
                    byte2 = Fetch_S();
                    byte1 += r_Y_S;
                    ushort2 = Mem_r_S((ushort)(byte1 | (byte2 << 8)));
                    if (byte1 < r_Y_S)
                    {
                        byte2++;
                        ushort2 = Mem_r_S((ushort)(byte1 | ((byte2) << 8)));
                        cpu_cycles_S++;
                    }
                    int1 = ushort2 | r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x01: //ORA (Indirect,X)
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    int1 = Mem_r_S((ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8))) | r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x11: //ORA (Indirect),Y
                    byte1 = Fetch_S();
                    ushort1 = (ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    int1 = Mem_r_S(ushort2) | r_A_S;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;
                //--- ORA END

                case 0x48: Mem_w_S((ushort)(r_SP_S-- | 0x100), r_A_S); break;//PHA
                case 0x08: Mem_w_S((ushort)(r_SP_S-- | 0x100), (byte)(GetFlag_S() | 0x30)); break;//PHP

                case 0x68://PLA
                    r_A_S = Mem_r_S((ushort)(++r_SP_S | 0x100));
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x28: SetFlag_S(Mem_r_S((ushort)(++r_SP_S | 0x100))); break;//PLP

                //----ROL begin
                case 0x2A://ROL acc //fix
                    ushort1 = (ushort)(r_A_S << 1);
                    if (flagC_S == 1) ushort1 |= 0x1;
                    if ((r_A_S & 0x80) != 0) flagC_S = 1; else flagC_S = 0;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if ((ushort1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    r_A_S = (byte)ushort1;
                    break;

                case 0x26://ROL zp //fix
                    byte2 = Fetch_S();
                    byte1 = NES_MEM_S[byte2];
                    ushort1 = (ushort)(byte1 << 1);
                    if (flagC_S == 1) ushort1 |= 0x1;
                    if ((byte1 & 0x80) != 0) flagC_S = 1; else flagC_S = 0;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if ((ushort1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    NES_MEM_S[byte2] = (byte)ushort1; //!!!!!
                    break;

                case 0x36://ROL zp,x
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = NES_MEM_S[byte2];
                    ushort1 = (ushort)(byte1 << 1);
                    if (flagC_S == 1) ushort1 |= 0x1;
                    if ((byte1 & 0x80) != 0) flagC_S = 1; else flagC_S = 0;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if ((ushort1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    NES_MEM_S[byte2] = (byte)ushort1; //!!!!!
                    break;

                case 0x2E://ROL abs fix
                    ushort2 = (ushort)((Fetch_S() | Fetch_S() << 8));
                    byte1 = Mem_r_S(ushort2);
                    Mem_w_S(ushort2, byte1);//dummy write fixed
                    ushort1 = (ushort)(byte1 << 1);
                    if (flagC_S == 1) ushort1 |= 0x1;
                    if ((byte1 & 0x80) != 0) flagC_S = 1; else flagC_S = 0;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if ((ushort1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    Mem_w_S(ushort2, (byte)ushort1); //!!!!!
                    break;

                case 0x3E://ROL abs,x fix
                    byte1 = Fetch_S(); //low
                    byte2 = Fetch_S(); //heigh
                    byte1 += r_X_S;
                    Mem_r_S((ushort)(byte1 | ((byte2) << 8)));
                    if (byte1 < r_X_S) byte2++;
                    byte3 = Mem_r_S((ushort)(byte1 | ((byte2) << 8))); //dummy read fixed 2017.01.17
                    Mem_w_S((ushort)(byte1 | (byte2 << 8)), byte3);
                    ushort1 = (ushort)(byte3 << 1);
                    if (flagC_S == 1) ushort1 |= 0x1;
                    if ((byte3 & 0x80) != 0) flagC_S = 1; else flagC_S = 0;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if ((ushort1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    Mem_w_S((ushort)(byte1 | (byte2 << 8)), (byte)ushort1);
                    break;
                //----ROL end

                //---- ROR begin
                case 0x6A://ROR acc
                    ushort1 = r_A_S;
                    if (flagC_S == 1) ushort1 |= 0x100;
                    if ((ushort1 & 0x01) > 0) flagC_S = 1; else flagC_S = 0;
                    ushort1 >>= 1;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (ushort1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    r_A_S = (byte)ushort1;
                    break;

                case 0x66://ROR zp
                    byte1 = Fetch_S();
                    ushort1 = NES_MEM_S[byte1];
                    if (flagC_S == 1) ushort1 |= 0x100;
                    if ((ushort1 & 0x01) > 0) flagC_S = 1; else flagC_S = 0;
                    ushort1 >>= 1;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (ushort1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    ushort1 = (byte)ushort1;
                    NES_MEM_S[byte1] = (byte)ushort1;
                    break;

                case 0x76://ROR zp,x
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    ushort1 = NES_MEM_S[byte1];
                    if (flagC_S == 1) ushort1 |= 0x100;
                    if ((ushort1 & 0x01) > 0) flagC_S = 1; else flagC_S = 0;
                    ushort1 >>= 1;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (ushort1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    ushort1 = (byte)ushort1;
                    NES_MEM_S[byte1] = (byte)ushort1;
                    break;

                case 0x6E://ROR abs
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = Mem_r_S(ushort2);
                    Mem_w_S(ushort2, (byte)ushort1);//dummy write fixed
                    if (flagC_S == 1) ushort1 |= 0x100;
                    if ((ushort1 & 0x01) > 0) flagC_S = 1; else flagC_S = 0;
                    ushort1 >>= 1;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (ushort1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    ushort1 = (byte)ushort1;
                    Mem_w_S(ushort2, (byte)ushort1);
                    break;

                case 0x7E://ROR abs,x
                    ushort3 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort3 + r_X_S);
                    Mem_r_S((ushort)((ushort3 & 0xFF00) | (ushort2 & 0x00FF))); // dummy read wrong page
                    ushort1 = Mem_r_S(ushort2);
                    Mem_w_S(ushort2, (byte)ushort1);//dummy write fixed
                    if (flagC_S == 1) ushort1 |= 0x100;
                    if ((ushort1 & 0x01) > 0) flagC_S = 1; else flagC_S = 0;
                    ushort1 >>= 1;
                    if ((ushort1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (ushort1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    ushort1 = (byte)ushort1;
                    Mem_w_S(ushort2, (byte)ushort1);
                    break;
                // ----ROR end

                case 0x40://RTI
                    Mem_r_S(r_PC_S);//dummy read fixed 2017.01.19

                    SetFlag_S(Mem_r_S((ushort)(++r_SP_S | 0x100)));
                    r_PC_S = (ushort)(Mem_r_S((ushort)(++r_SP_S | 0x100)) | (Mem_r_S((ushort)(++r_SP_S | 0x100)) << 8));
                    break;

                case 0x60://RTS

                    Mem_r_S(r_PC_S);//dummy read fixed 2017.01.19

                    r_PC_S = (ushort)((Mem_r_S((ushort)(++r_SP_S | 0x100)) | (Mem_r_S((ushort)(++r_SP_S | 0x100)) << 8)) + 1);
                    break;

                //--- SBC BEGIN
                case 0xE9: //SBC  Immediate
                    byte1 = (byte)(Fetch_S() ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xE5: //SBC  Zero Page
                    byte1 = (byte)(NES_MEM_S[Fetch_S()] ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xF5://SBC Zero Page,X
                    byte1 = (byte)(NES_MEM_S[(byte)(Fetch_S() + r_X_S)] ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xED: //SBC Absolute fix
                    byte1 = (byte)(Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8))) ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xFD: //SBC  Absolute,X
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = (byte)(Mem_r_S(ushort2) ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xF9: //SBC  Absolute,Y
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = (byte)(Mem_r_S(ushort2) ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xE1: //SBC (Indirect,X)
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = (byte)(Mem_r_S((ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8))) ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xF1: //SBC (Indirect),Y
                    byte2 = Fetch_S();
                    ushort1 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF)));
                        cpu_cycles_S++;
                    }
                    byte1 = (byte)(Mem_r_S(ushort2) ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;
                //--- SBC END
                case 0x38: flagC_S = 1; break; //SEC
                case 0xF8: flagD_S = 1; break; // SED NES 6502 此 FLAG 無作用
                case 0x78: flagI_S = 1; break; //SEI
                case 0x85: NES_MEM_S[Fetch_S()] = r_A_S; break;//STA zp
                case 0x95: NES_MEM_S[(byte)(Fetch_S() + r_X_S)] = r_A_S; break;//STA zp,x
                case 0x8D: Mem_w_S((ushort)(Fetch_S() | (Fetch_S() << 8)), r_A_S); break;//STA abs
                case 0x9D:  //STA abs,x
                    byte1 = Fetch_S(); //low
                    byte2 = Fetch_S(); //heigh
                    byte1 += r_X_S;

                    Mem_r_S((ushort)(byte1 | ((byte2) << 8)));
                    //
                    if (byte1 < r_X_S) byte2++;
                    Mem_w_S((ushort)(byte1 | ((byte2) << 8)), r_A_S);//dummy read fiexed 2017.01.17
                    //
                    break;
                case 0x99: //STA abs,Y
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_Y_S);
                    Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF))); // dummy read (always)
                    Mem_w_S(ushort2, r_A_S);
                    break;

                case 0x81://STA (indirect,x)
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    Mem_w_S((ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8)), r_A_S);
                    break;

                case 0x91://STA (indirect),y
                    byte3 = Fetch_S();
                    byte1 = Mem_r_S(byte3++); //low
                    byte2 = Mem_r_S(byte3); //heigh
                    byte1 += r_Y_S;
                    Mem_r_S((ushort)(byte1 | ((byte2) << 8)));

                    //
                    if (byte1 < r_Y_S) byte2++;
                    Mem_w_S((ushort)(byte1 | ((byte2) << 8)), r_A_S);//dummy read fiexed 2017.01.17
                    //
                    break;

                case 0x86: NES_MEM_S[Fetch_S()] = r_X_S; break; //STX zp
                case 0x96: NES_MEM_S[(byte)(Fetch_S() + r_Y_S)] = r_X_S; break; //STX zp,y
                case 0x8E: Mem_w_S((ushort)(Fetch_S() | (Fetch_S() << 8)), r_X_S); break; //STX abs //fixed 1/3
                case 0x84: NES_MEM_S[Fetch_S()] = r_Y_S; break;//STY  zp
                case 0x94: NES_MEM_S[(byte)(Fetch_S() + r_X_S)] = r_Y_S; break;//STY zp,x
                case 0x8C: Mem_w_S((ushort)(Fetch_S() | (Fetch_S() << 8)), r_Y_S); break;//STY abs

                case 0xAA: //TAX
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    r_X_S = r_A_S;
                    break;

                case 0xA8://TAY
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    r_Y_S = r_A_S;
                    break;

                case 0xBA://TSX
                    if ((r_SP_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_SP_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    r_X_S = r_SP_S;
                    break;

                case 0x8A://TXA
                    if ((r_X_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    r_A_S = r_X_S;
                    break;

                case 0x9A: r_SP_S = r_X_S; break; //TXS

                case 0x98: //TYA
                    if ((r_Y_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (r_Y_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    r_A_S = r_Y_S;
                    break;
#if illegal
                #region illagel code
                // http://visual6502.org/wiki/index.php?title=6502_all_256_Opcodes
                // http://macgui.com/kb/article/46

                //do nothing
                case 0x1A:
                case 0x3A:
                case 0x5A:
                case 0x7A:
                case 0xDA:
                case 0xFA:
                    break;

                case 0x80:
                case 0x82:
                case 0x89:
                case 0xC2:
                case 0xE2:
                case 0x04:
                case 0x44:
                case 0x64:
                case 0x14:
                case 0x34:
                case 0x54:
                case 0xD4:
                case 0xF4:
                case 0x74:
                    r_PC_S += 1;
                    break;

                case 0x0C: // NOP abs (unofficial)
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    Mem_r_S(ushort1); // read and discard
                    break;

                case 0x1C: case 0x3C: case 0x5C: case 0x7C: case 0xDC: case 0xFC: // NOP abs,X (unofficial)
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort2 = (ushort)(ushort1 + r_X_S);
                    if ((ushort1 & 0xff00) != (ushort2 & 0xff00))
                    {
                        Mem_r_S((ushort)((ushort1 & 0xFF00) | (ushort2 & 0x00FF))); // dummy read wrong page
                        cpu_cycles_S++;
                    }
                    Mem_r_S(ushort2); // read and discard
                    break;

                case 0x6B:
                    byte1 = Fetch_S();
                    r_A_S = (byte)(((byte1 & r_A_S) >> 1) | (((byte)flagC_S) << 7));
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if ((r_A_S & 0x40) > 0) flagC_S = 1; else flagC_S = 0;
                    if (((r_A_S << 1 ^ r_A_S) & 0x40) > 0) flagV_S = 1; else flagV_S = 0;
                    break;

                case 0x0B: //ANC
                case 0x2B: //ANC
                    r_A_S &= Fetch_S();
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    flagN_S = flagC_S;
                    break;

                case 0x4B: //ALR
                    r_A_S &= Fetch_S();
                    if ((r_A_S & 0x1) != 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S >>= 1;
                    if ((r_A_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0xEB: //illegal sbc imm
                    byte1 = (byte)(Fetch_S() ^ 0xFF);
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if (int1 > 0xff) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0x03: //SLO (  ASL M THEN (M "OR" A) -> A,M  )
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    ushort1 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S |= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x07: //SLO (  ASL M THEN (M "OR" A) -> A,M  )
                    byte2 = Fetch_S();
                    byte1 = NES_MEM_S[byte2];
                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    NES_MEM_S[byte2] = byte1;
                    r_A_S |= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x13: //SLO (  ASL M THEN (M "OR" A) -> A,M  )
                    byte2 = Fetch_S();
                    ushort2 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    Mem_w_S((ushort)(ushort1), byte1);
                    r_A_S |= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x17: //SLO (  ASL M THEN (M "OR" A) -> A,M  )
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = NES_MEM_S[byte2];
                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    NES_MEM_S[byte2] = byte1;
                    r_A_S |= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x1B: //SLO (  ASL M THEN (M "OR" A) -> A,M  )
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S |= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x0F: //SLO (  ASL M THEN (M "OR" A) -> A,M  )
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S |= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x1F: //SLO (  ASL M THEN (M "OR" A) -> A,M  )
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_X_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 <<= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S |= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x23: //RLA    ( ROL M  THEN (M "AND" A) -> A )
                    byte3 = (byte)(Fetch_S() + r_X_S);
                    ushort1 = (ushort)(NES_MEM_S[byte3++] | (NES_MEM_S[byte3] << 8));
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 << 1) | flagC_S);
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S &= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x27: //RLA    ( ROL M  THEN (M "AND" A) -> A )
                    byte3 = Fetch_S();
                    byte2 = NES_MEM_S[byte3];
                    byte1 = (byte)((byte2 << 1) | flagC_S);
                    NES_MEM_S[byte3] = byte1;
                    if ((byte2 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S &= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x2F:// RLA
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 << 1) | flagC_S);
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S &= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x3F://RLA
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_X_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 << 1) | flagC_S);
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S &= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x3B://RLA
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 << 1) | flagC_S);
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S &= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x33: //RLA    ( ROL M  THEN (M "AND" A) -> A )
                    byte3 = Fetch_S();
                    ushort2 = (ushort)(NES_MEM_S[byte3++] | (NES_MEM_S[byte3] << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)(byte2 << 1);
                    byte1 |= (byte)(flagC_S);
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S &= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x37: //RLA    ( ROL M  THEN (M "AND" A) -> A )
                    byte3 = (byte)(Fetch_S() + r_X_S);
                    byte2 = NES_MEM_S[byte3];
                    byte1 = (byte)(byte2 << 1);
                    byte1 |= (byte)(flagC_S);
                    NES_MEM_S[byte3] = byte1;
                    if ((byte2 & 0x80) > 0) flagC_S = 1; else flagC_S = 0;
                    r_A_S &= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x43://SRE (LSR M  THEN (M "EOR" A) -> A )
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    ushort1 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S ^= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x47://SRE (LSR M  THEN (M "EOR" A) -> A )
                    byte2 = Fetch_S();
                    byte1 = NES_MEM_S[byte2];
                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    NES_MEM_S[byte2] = byte1;
                    r_A_S ^= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x4F://SRE (LSR M  THEN (M "EOR" A) -> A )
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S ^= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x5F://SRE
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_X_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 1) != 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S ^= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x5B://SRE
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 1) != 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S ^= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x53://SRE (LSR M  THEN (M "EOR" A) -> A )
                    byte2 = Fetch_S();
                    ushort2 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    Mem_w_S(ushort1, byte1);
                    r_A_S ^= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x57://SRE (LSR M  THEN (M "EOR" A) -> A )
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = NES_MEM_S[byte2];
                    if ((byte1 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    byte1 >>= 1;
                    NES_MEM_S[byte2] = byte1;
                    r_A_S ^= byte1;
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) > 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0x63:// RRA (ROR M THEN (A + M + C) -> A  )  ok
                    byte3 = (byte)(Fetch_S() + r_X_S);
                    ushort1 = (ushort)(NES_MEM_S[byte3++] | (NES_MEM_S[byte3] << 8));
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 >> 1) | ((flagC_S == 0) ? 0 : 0x80));
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    if ((int1 >> 8) > 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x67:// RRA (ROR M THEN (A + M + C) -> A  ) ok
                    byte3 = Fetch_S();
                    byte2 = NES_MEM_S[byte3];
                    byte1 = (byte)((byte2 >> 1) | ((flagC_S == 0) ? 0 : 0x80));
                    NES_MEM_S[byte3] = byte1;
                    if ((byte2 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    if ((int1 >> 8) > 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x6F://RRA
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));//ok
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 >> 1) | ((flagC_S == 0) ? 0 : 0x80));
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    if ((int1 >> 8) > 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x73:// RRA (ROR M THEN (A + M + C) -> A  ) ok
                    byte3 = Fetch_S();
                    ushort2 = (ushort)(NES_MEM_S[byte3++] | (NES_MEM_S[byte3] << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 >> 1) | ((flagC_S == 0) ? 0 : 0x80));
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    if ((int1 >> 8) > 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x77:// RRA (ROR M THEN (A + M + C) -> A  ) ok
                    byte3 = (byte)(Fetch_S() + r_X_S);
                    byte2 = NES_MEM_S[byte3];
                    byte1 = (byte)((byte2 >> 1) | ((flagC_S == 0) ? 0 : 0x80));
                    NES_MEM_S[byte3] = byte1;
                    if ((byte2 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    if ((int1 >> 8) > 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x7B:// RRA (ROR M THEN (A + M + C) -> A  )
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 >> 1) | ((flagC_S == 0) ? 0 : 0x80));
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    if ((int1 >> 8) > 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x7F: //RRA
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_X_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte2 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte2);//dummy write fixed

                    byte1 = (byte)((byte2 >> 1) | ((flagC_S == 0) ? 0 : 0x80));
                    Mem_w_S(ushort1, byte1);
                    if ((byte2 & 1) > 0) flagC_S = 1; else flagC_S = 0;
                    int1 = r_A_S + byte1 + flagC_S;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if (((int1 ^ r_A_S) & (int1 ^ byte1) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    r_A_S = (byte)int1;
                    if ((int1 >> 8) > 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    break;

                case 0x83://SAX ( (A "AND" (MSB(adr)+1)  "AND" X) -> M
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    Mem_w_S((ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8)), (byte)(r_X_S & r_A_S));
                    break;

                case 0x87://SAX ( (A "AND" (MSB(adr)+1)  "AND" X) -> M
                    Mem_w_S(Fetch_S(), (byte)(r_X_S & r_A_S));
                    break;

                case 0x8F://SAX ( (A "AND" (MSB(adr)+1)  "AND" X) -> M
                    Mem_w_S((ushort)(Fetch_S() | (Fetch_S() << 8)), (byte)(r_X_S & r_A_S));
                    break;

                case 0x9C://SHY
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte1 = (byte)(r_Y_S & (((ushort1 & 0xff00) >> 8) + 1));
                    ushort1 = (ushort)((ushort1 & 0xff00) | (byte)(ushort1 + r_X_S));
                    Mem_r_S(ushort1); // dummy read at wrong-page address
                    if ((ushort1 & 0xff) < r_X_S) ushort1 = (ushort)((ushort1 & 0xff) | (byte1 << 8));
                    Mem_w_S(ushort1, byte1);
                    break;

                case 0x9E://SHX
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte1 = (byte)(r_X_S & (((ushort1 & 0xff00) >> 8) + 1));
                    ushort1 = (ushort)((ushort1 & 0xff00) | (byte)(ushort1 + r_Y_S));
                    Mem_r_S(ushort1); // dummy read at wrong-page address
                    if ((ushort1 & 0xff) < r_Y_S) ushort1 = (ushort)((ushort1 & 0xff) | (byte1 << 8));
                    Mem_w_S(ushort1, byte1);
                    break;

                case 0x9B://TAS/SHS abs,Y - SP = A & X, write SP & (H+1)
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    r_SP_S = (byte)(r_A_S & r_X_S);
                    byte1 = (byte)(r_SP_S & (((ushort1 & 0xff00) >> 8) + 1));
                    ushort1 = (ushort)((ushort1 & 0xff00) | (byte)(ushort1 + r_Y_S));
                    Mem_r_S(ushort1); // dummy read at wrong-page address
                    if ((ushort1 & 0xff) < r_Y_S) ushort1 = (ushort)((ushort1 & 0xff) | (byte1 << 8));
                    Mem_w_S(ushort1, byte1);
                    break;

                case 0x9F://SHA/AXA abs,Y - write A & X & (H+1)
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte1 = (byte)(r_A_S & r_X_S & (((ushort1 & 0xff00) >> 8) + 1));
                    ushort1 = (ushort)((ushort1 & 0xff00) | (byte)(ushort1 + r_Y_S));
                    Mem_r_S(ushort1); // dummy read at wrong-page address
                    if ((ushort1 & 0xff) < r_Y_S) ushort1 = (ushort)((ushort1 & 0xff) | (byte1 << 8));
                    Mem_w_S(ushort1, byte1);
                    break;

                case 0x93://SHA/AXA (ind),Y - write A & X & (H+1)
                    byte2 = Fetch_S();
                    ushort2 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    byte1 = (byte)(r_A_S & r_X_S & (((ushort2 >> 8) & 0xFF) + 1));
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    if ((ushort1 & 0xFF00) != (ushort2 & 0xFF00))
                        ushort1 = (ushort)((ushort1 & 0xFF) | (byte1 << 8));
                    Mem_w_S(ushort1, byte1);
                    break;

                case 0x97://SAX ( (A "AND" (MSB(adr)+1)  "AND" X) -> M
                    NES_MEM_S[(byte)(Fetch_S() + r_Y_S)] = (byte)(r_X_S & r_A_S);
                    break;

                case 0xB7://SAX ( (A "AND" (MSB(adr)+1)  "AND" X) -> M
                    r_X_S = r_A_S = NES_MEM_S[(byte)(Fetch_S() + r_Y_S)];
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_X_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xA3://LAX
                    byte1 = (byte)(Fetch_S() + r_X_S);
                    r_X_S = r_A_S = Mem_r_S((ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8)));
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_X_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xA7://LAX
                    r_X_S = r_A_S = NES_MEM_S[Fetch_S()];
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_X_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xAB://LAX
                    r_X_S = r_A_S = Fetch_S();
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_X_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xAF://LAX
                    r_X_S = r_A_S = Mem_r_S((ushort)(Fetch_S() | (Fetch_S() << 8)));
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_X_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xBF://LAX
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    if ((ushort1 & 0xFF00) != (ushort2 & 0xFF00)) // page cross
                        Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    r_X_S = r_A_S = Mem_r_S(ushort1);
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_X_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xB3://LAX
                    byte1 = Fetch_S();
                    ushort2 = (ushort)(NES_MEM_S[byte1++] | (NES_MEM_S[byte1] << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    if ((ushort1 & 0xFF00) != (ushort2 & 0xFF00)) // page cross
                        Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    r_X_S = r_A_S = Mem_r_S(ushort1);
                    if (r_X_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_X_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xBB://LAS/LAR abs,Y - A = X = SP = M & SP
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    if ((ushort1 & 0xFF00) != (ushort2 & 0xFF00)) // page cross
                        Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    r_A_S = r_X_S = r_SP_S = (byte)(Mem_r_S(ushort1) & r_SP_S);
                    if (r_A_S == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((r_A_S & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xCB:// AXS
                    int1 = (r_A_S & r_X_S) - Fetch_S();
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    if ((byte)int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((~int1 >> 8) != 0) flagC_S = 1; else flagC_S = 0;
                    r_X_S = (byte)int1;
                    break;

                case 0xC3: //DCP
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    ushort1 = (ushort)((NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8)));
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    Mem_w_S(ushort1, --byte1);
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((~int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xC7: //DCP
                    byte2 = Fetch_S();
                    byte1 = NES_MEM_S[byte2];
                    NES_MEM_S[byte2] = --byte1;
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((~int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xCF: //DCP
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    Mem_w_S(ushort1, --byte1);
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((~int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xDF: //DCP
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_X_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    Mem_w_S(ushort1, --byte1);
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((~int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xD3: //DCP
                    byte2 = Fetch_S();
                    ushort2 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    ushort3 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort3 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort3);

                    Mem_w_S(ushort3, byte1);//dummy write fixed

                    Mem_w_S(ushort3, --byte1);
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((~int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xD7: //DCP
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = NES_MEM_S[byte2];
                    NES_MEM_S[byte2] = --byte1;
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((~int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xDB:// DCP
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    Mem_w_S(ushort1, --byte1);
                    int1 = r_A_S - byte1;
                    if (int1 == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((~int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    break;

                case 0xE3://ISC
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    ushort3 = (ushort)((NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8)));
                    byte1 = Mem_r_S(ushort3);

                    Mem_w_S(ushort3, byte1);//dummy write fixed

                    Mem_w_S(ushort3, ++byte1);
                    int1 = r_A_S + (byte1 ^ 0xff) + flagC_S;
                    if (((int1 ^ r_A_S) & (int1 ^ (byte1 ^ 0xff)) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xE7://ISC
                    byte2 = Fetch_S();
                    byte1 = NES_MEM_S[byte2];
                    NES_MEM_S[byte2] = ++byte1;
                    int1 = r_A_S + (byte1 ^ 0xff) + flagC_S;
                    if (((int1 ^ r_A_S) & (int1 ^ (byte1 ^ 0xff)) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xEF://ISC
                    ushort1 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    Mem_w_S(ushort1, ++byte1);
                    int1 = r_A_S + (byte1 ^ 0xff) + flagC_S;
                    if (((int1 ^ r_A_S) & (int1 ^ (byte1 ^ 0xff)) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xF3://ISC
                    byte2 = Fetch_S();
                    ushort2 = (ushort)(NES_MEM_S[byte2++] | (NES_MEM_S[byte2] << 8));
                    ushort3 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort3 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort3);

                    Mem_w_S(ushort3, byte1);//dummy write fixed

                    Mem_w_S(ushort3, ++byte1);
                    int1 = r_A_S + (byte1 ^ 0xff) + flagC_S;
                    if (((int1 ^ r_A_S) & (int1 ^ (byte1 ^ 0xff)) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xF7://ISC
                    byte2 = (byte)(Fetch_S() + r_X_S);
                    byte1 = NES_MEM_S[byte2];
                    NES_MEM_S[byte2] = ++byte1;
                    int1 = r_A_S + (byte1 ^ 0xff) + flagC_S;
                    if (((int1 ^ r_A_S) & (int1 ^ (byte1 ^ 0xff)) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xFB://ISC
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_Y_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    Mem_w_S(ushort1, ++byte1);
                    int1 = r_A_S + (byte1 ^ 0xff) + flagC_S;
                    if (((int1 ^ r_A_S) & (int1 ^ (byte1 ^ 0xff)) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;

                case 0xFF://ISC
                    ushort2 = (ushort)(Fetch_S() | (Fetch_S() << 8));
                    ushort1 = (ushort)(ushort2 + r_X_S);
                    Mem_r_S((ushort)((ushort2 & 0xFF00) | (ushort1 & 0x00FF))); // dummy read at wrong-page address
                    byte1 = Mem_r_S(ushort1);

                    Mem_w_S(ushort1, byte1);//dummy write fixed

                    Mem_w_S(ushort1, ++byte1);
                    int1 = r_A_S + (byte1 ^ 0xff) + flagC_S;
                    if (((int1 ^ r_A_S) & (int1 ^ (byte1 ^ 0xff)) & 0x80) != 0) flagV_S = 1; else flagV_S = 0;
                    if ((int1 & 0xff) == 0) flagZ_S = 1; else flagZ_S = 0;
                    if ((int1) >> 8 != 0) flagC_S = 1; else flagC_S = 0;
                    if ((int1 & 0x80) != 0) flagN_S = 1; else flagN_S = 0;
                    r_A_S = (byte)int1;
                    break;
                #endregion
#endif
                default: ShowError_S("unkonw opcode ! - 0x" + opcode_S.ToString("X2")); break;
            }

            // SP-2: Batch tick — advance PPU once, APU in tight loop (no per-cycle PPU branch)
            ppu_x_S += cpu_cycles_S * 3;
            while (ppu_x_S >= 341) { ppu_x_S -= 341; end_scanline_S(); }

            // SP-13: Batch APU step — eliminate per-cycle function call overhead when audio off.
            // Bulk-update counters; frame counter fires at most once (7457 cycles >> max 7 cycles/instr).
            if (!AudioEnabled_S)
            {
                apucycle_S += cpu_cycles_S;
                framectrdiv_S -= cpu_cycles_S;
                if (framectrdiv_S <= 0)
                {
                    clockframecounter_S();
                    framectrdiv_S += (ctrmode_S == 4) ? frameReload4_S[framectr_S] : frameReload5_S[framectr_S];
                }
            }
            else
            {
                for (int i = 0; i < cpu_cycles_S; i++) apu_step_S();
            }
        }
    }
}
