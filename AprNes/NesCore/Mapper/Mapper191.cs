namespace AprNes
{
    ...
    // bankmodes) preserved.
    unsafe public class Mapper191 : IMapper
    {
        ...
        // ── Variant-specific: 2KB CHR-RAM at banks 0x80..0xFF (128 banks map to 2 RAM pages with wrap) ──
        const int CHR_RAM_FIRST = 0x80;
        const int CHR_RAM_LAST  = 0xFF;
        const int CHR_RAM_SIZE  = 2048;
        const int CHR_RAM_PAGES = 2;   // 1KB pages
        ...
        // Per-1KB-slot pointer: check variant range first (wraps modulo RAM pages), else CHR-ROM.
        byte* BankPtr(int bank1k)
        {
            if (bank1k >= CHR_RAM_FIRST && bank1k <= CHR_RAM_LAST)
            {
                int ramPage = (bank1k - CHR_RAM_FIRST) % CHR_RAM_PAGES;
                return chrRamBuffer + (ramPage << 10);
            }
